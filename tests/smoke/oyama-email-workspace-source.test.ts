...
describe("OyamaEmail workspace source contract", () => {
  ...
  it("keeps the workspace API-backed and includes core sidebar navigation actions", () => {
    ...
    expect(workspace).toContain("Select Audience");
    expect(workspace).toContain("Save Audience");
    expect(workspace).toContain("/oyama-email/campaigns/${row.id}?tab=audience");
    expect(workspace).toContain("Open List Builder &amp; Contact Tags");
    ...
    expect(contactsManager).toContain("Audience List Builder");
    expect(contactsManager).toContain("Base list saved");
    expect(contactsManager).toContain("Unsaved changes");
    expect(contactsManager).toContain("Every future email, letter, label, and export reads its latest saved membership.");
    ...
    expect(contactsManager).toContain("Removed ${contactName(row)} from ${targetListName}");
    expect(contactsManager).toContain("Include Church tags");
    expect(contactsManager).toContain("Exclude Church tags");
    expect(contactsManager).toContain("removeConstituentFromList(row, membershipListId)");
    expect(contactsManager).toContain("Remove filtered (");
    expect(contactsManager).toContain("selectAudienceList(event.target.value, true)");
    ...
    expect(builder).toContain("Choose one block. Its single editing panel will open on the right.");
    expect(builder).toContain("Add content");
    expect(builder).toContain('aria-label="Add email block"');
    ...
    expect(builder).toContain("Organization header is included automatically");
    expect(builder).toContain("Footer and compliance bar are managed automatically");
    expect(builder).toContain("Use In-body Header only when you intentionally want a second header.");
expect(builder).toContain("Ready-made layouts");
    expect(builder).toContain("Two Image Cards");
    expect(builder).toContain("Image + Copy");
    expect(builder).toContain("Upload image for this slot");
    expect(builder).toContain("createColumnTemplate");
    expect(builder).toContain("email-block-library-title");
    expect(builder).toContain("BLOCK_LIBRARY_DRAWERS");
    expect(builder).toContain("Builder inspector");
    expect(builder).toContain("Edit this block in the inspector on the right.");
    expect(builder).toContain("Add a block");
    expect(builder).toContain("Browse reusable email sections");
    ...
    expect(workspace).toContain("templateSnapshot:");
    expect(workspace).toContain("Your reusable template remains in the template library.");
