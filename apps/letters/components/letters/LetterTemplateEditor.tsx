...
import FormLetterRichEditor, { type LetterEditorCommands, type LetterInsertBlockKind } from "@/components/letters/FormLetterRichEditor";
import LetterBuilderIcon, { type LetterBuilderIconName } from "@/components/letters/LetterBuilderIcon";
import WorkspaceSetupModal from "@/components/ui/WorkspaceSetupModal";
import { bodyToPrintLayout, parsePrintLayout } from "@/components/letters/print-layout-utils";
import {
...
} from "@/components/letters/letter-branding-rendering";
import {
...
import type { FooterPreset, HeaderPreset, MergeFieldSection, PrintLayoutDocument, SignatureBlock } from "@/components/letters/types";
...
export default function LetterTemplateEditor({ templateId, fullScreen = false, initialPanel = "document" }: LetterTemplateEditorProps) {
  ...
  /** Saves first, then opens generated-document history in the unified generator activity rail. */
  async function openGenerateHistoryWorkspace() {
    const currentId = await saveTemplate({ silent: true });
    if (!currentId) return;
    const params = new URLSearchParams({ templateId: currentId, tab: "activity" });
    router.push(`/oyama-letters/generate?${params.toString()}`);
  }
            ...
            <div className="relative shrink-0">
              ...
              {isMoreMenuOpen && (
                <div className="absolute right-0 z-50 mt-2 w-56 rounded-xl border border-gray-200 bg-white p-2 text-sm shadow-xl">
                  ...
                  <MoreMenuButton label="View Generation History" onClick={() => void openGenerateHistoryWorkspace()} />
                </div>
              )}
            </div>
          </div>
