...
interface OShareviewReportBlueprint {
  ...
  name: string;
  description: string;
  module: "donor" | "events" | "ogentic" | "admin";
  tool: string;
...
function normalizeOShareviewBlueprints(config: unknown): OShareviewReportBlueprint[] {
  ...
  const modules = new Set(["donor", "events", "ogentic", "admin"]);
...
router.post("/oshareview-blueprints", async (req, res) => {
  ...
  const nextBlueprint: OShareviewReportBlueprint = {
    ...
    module: req.body?.module === "events" || req.body?.module === "ogentic" || req.body?.module === "admin" ? req.body.module : "donor",
 ...
 * Donor operational reporting for administrators.
 */
router.get("/admin-summary", async (req, res) => {
  ...
  const { year, yearRange, donationDateFilter } = await parseReportScope(req.query, organizationId);
  const [
    totalConstituents,
    activeDonors,
    lapsedDonors,
    ...
    prisma.donation.findMany({ where: completedDonationWhere(organizationId, donationDateFilter), select: { amount: true, date: true } }),
    prisma.constituent.findMany({ where: { organizationId, email: { not: null } }, select: { email: true } }),
    prisma.constituent.findMany({
      where: { organizationId, OR: [{ email: null }, { phone: null }, { donorStatus: "LAPSED" }] },
      orderBy: [{ totalLifetimeGiving: "desc" }, { updatedAt: "desc" }],
      take: 120,
      select: { id: true, firstName: true, lastName: true, email: true, phone: true, donorStatus: true, totalLifetimeGiving: true, lastGiftDate: true },
    }),
    prisma.constituent.findMany({ where: { organizationId, createdAt: yearRange }, select: { createdAt: true } }),
    prisma.donation.findMany({ where: completedDonationWhere(organizationId, yearRange), select: { date: true, amount: true } }),
  ]);

  const donorEmailCounts = new Map<string, number>();
  donorEmailRows.forEach(({ email }) => {
    const normalized = normalizeEmail(email);
    if (normalized) donorEmailCounts.set(normalized, (donorEmailCounts.get(normalized) ?? 0) + 1);
  });
  const donorDuplicateEmails = new Set(Array.from(donorEmailCounts.entries()).filter(([, count]) => count > 1).map(([email]) => email));
  ...
  constituentTrendRows.forEach((row) => { monthlyTrend[row.createdAt.getMonth()].newConstituents += 1; });
  donationTrendRows.forEach((row) => {
    const month = row.date.getMonth();
    monthlyTrend[month].donations = centsToMoney(moneyToCents(monthlyTrend[month].donations) + moneyToCents(row.amount));
  });

  const riskRows = donorRiskCandidates.map((row) => {
    const normalized = normalizeEmail(row.email);
    return {
      kind: "donor" as const,
      id: row.id,
      name: `${row.firstName ?? ""} ${row.lastName ?? ""}`.trim() || "Unknown donor",
      email: row.email,
      status: row.donorStatus,
      missingContact: !row.email || !row.phone,
      duplicateEmail: normalized ? donorDuplicateEmails.has(normalized) : false,
  ...
  res.json({
    generatedAt: new Date().toISOString(),
    year,
    donorTotals: { totalConstituents, activeDonors, lapsedDonors, majorDonors, newDonorsThisYear, missingContact: donorMissingContact, duplicateEmailCount: donorDuplicateEmails.size, totalGiftVolume, averageGift },
    monthlyTrend,
    alerts: [{
