/**
 * PWA utility routes — served by Express so that Nginx's /api/ → Express proxy
 * can deliver them in production (Next.js API routes under /api/ are unreachable
 * externally because Nginx proxies /api/ to this Express process on port 4000).
 *
 * Routes:
 *  GET /api/pwa/icon?size=192   — generates an Oyama-branded PNG icon
 */
...
// ─── Icon generation (pure Node.js, no external deps) ────────────────────────

const BG_R = 22, BG_G = 163, BG_B = 74; // #16a34a — Oyama brand green
...
/** GET /api/pwa/icon?size=192 — no auth required; public manifest asset */
router.get("/icon", (_req: Request, res: Response) => {
  const sizeParam = _req.query["size"];
  ...
  res.set({
    ...
    // Cache 7 days — icon doesn't change between deploys
