...
function TopCampaignsCard({ campaigns, loading }: { campaigns: CampaignSummary[]; loading: boolean }) {
  ...
  return (
    <CRMCard padding="lg" className="min-h-[21rem]">
      <SectionHeader
        title="Top Campaigns"
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {topCampaigns.map((campaign) => {
            ...
            return (
              <Link key={campaign.id} href={`/campaigns/${campaign.id}`} className="block rounded-xl transition-colors hover:bg-slate-50">
...
function TasksDueSoonCard({ tasks, loading }: { tasks: TaskPreview[]; loading: boolean }) {
  return (
    <CRMCard padding="lg" className="min-h-[18rem]">
      <SectionHeader
...
export default function DonorDashboardVisualRefresh({
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold tracking-tight text-slate-950">
            {greeting}, {name.split(" ")[0] || name} <span aria-hidden="true">👋</span>
          </h1>
          <p className="mt-1 text-sm text-slate-600">Here&apos;s what&apos;s happening with your ministry today.</p>
          <button
            type="button"
            onClick={onRefresh}
            className="mt-2 inline-flex items-center gap-1.5 text-xs font-medium text-slate-500 hover:text-emerald-700"
          >
            ...
            {loading ? "Refreshing..." : dataThroughLabel}
          </button>
        </div>
        <button
          type="button"
          onClick={onCustomize}
          className="inline-flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 shadow-sm transition-colors hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700"
        >
          <DashboardIcon name="chart" className="h-4 w-4" />
          Customize Dashboard
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(20rem,0.95fr)_minmax(20rem,0.95fr)]">
        <CRMCard padding="lg" className="min-h-[24rem]">
          <SectionHeader
            title="Giving Overview"
            action={<span className="rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-xs font-semibold text-slate-600">{reportingYearMode === "fiscal" ? "Fiscal YTD" : "Year to Date"}</span>}
          >
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2 2xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)_minmax(20rem,0.9fr)_minmax(20rem,0.9fr)]">
        <CRMCard padding="lg" className="min-h-[18rem]">
          <SectionHeader
        ...
        <CRMCard padding="lg" className="min-h-[18rem]">
          <SectionHeader title="Donor Retention" action={<span className="rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-xs font-semibold text-slate-600">All Donors</span>}>
            {monthTrendLabel}
          </SectionHeader>
          <DonorRetention
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {QUICK_ACTIONS.map((action) => (
              <Link
                key={action.label}
                href={action.href}
                className="group flex min-h-14 items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium text-slate-700 transition-all hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700"
              >
                <DashboardIcon name={action.icon} className="h-4 w-4 shrink-0 text-slate-500 group-hover:text-emerald-700" />
                <span className="min-w-0 flex-1 truncate">{action.label}</span>
                {action.badge ? <span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] font-semibold text-slate-500">{action.badge}</span> : null}
              </Link>
            ))}
          </div>
