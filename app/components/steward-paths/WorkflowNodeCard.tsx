/**
 * WorkflowNodeCard renders one action/branch card in the visual canvas.
 * Styled for the new SaaS builder presentation while keeping all interactions.
 */
"use client";
...
function nodeTypeLabel(kind: string): string {
  ...
  if (kind.startsWith("livecom.")) return "LIVECOM";
...
function nodeIconStyle(kind: string): string {
  ...
  if (kind.startsWith("livecom.")) return "bg-cyan-100 text-cyan-700";
...
function displayNodeTitle(node: WorkflowNode, fallback: string): string {
  ...
  const replacements: Record<string, string> = {
    ...
    "logic.retention_risk_condition": "Retention risk split",
    "donor.add_tag": "Add Tag",
    "donor.set_retention_stage": "Set Retention Stage",
    "livecom.send_message": "LiveCom Message",
    "livecom.wait_for_reply": "Wait for Reply",
    "livecom.route_to_staff": "Route to Staff",
...
function nodeBorderStyle(kind: string): string {
  if (kind.startsWith("trigger.")) return "border-emerald-300 bg-emerald-50/45";
  if (kind.startsWith("timing.")) return "border-violet-300 bg-violet-50/45";
  if (kind.startsWith("logic.")) return "border-violet-300 bg-violet-50/45";
  if (kind.startsWith("email.")) return "border-blue-300 bg-blue-50/45";
  if (kind.startsWith("task.")) return "border-amber-300 bg-amber-50/45";
  if (kind.startsWith("livecom.")) return "border-cyan-300 bg-cyan-50/45";
  if (kind.startsWith("donor.")) return "border-emerald-300 bg-emerald-50/45";
  if (kind.startsWith("safety.")) return "border-rose-300 bg-rose-50/45";
...
export default function WorkflowNodeCard({
  ...
  const selectedClass = isSelected
    ? "border-blue-500 ring-2 ring-blue-500/20 shadow-[0_10px_24px_rgba(37,99,235,0.14)]"
    : `${nodeBorderStyle(node.kind)} hover:border-slate-400 hover:shadow-md`;
  ...
  function updateFreeDrag(event: PointerEvent<HTMLDivElement>) {
    ...
    onFreeMove(node.id, { x: snapToGrid(nextX), y: snapToGrid(nextY) });
  }
  ...
  return (
    <div
      className={`group relative cursor-grab touch-none rounded-xl border ${compact ? "px-3 py-2" : "px-3.5 py-2.5"} transition-[border,box-shadow,background-color] active:cursor-grabbing ${selectedClass}`}
    ...
    >
      <span className="sr-only">Step {numberLabel}</span>

      {isSelected ? (
        <div className="absolute right-2 top-2 z-30 flex items-center gap-1 rounded-md border border-slate-200 bg-white/95 p-1 shadow-sm">
          <button
            type="button"
            title="Move step up"
            onClick={(event) => { event.stopPropagation(); onMoveUp(node.id); }}
            disabled={!canMoveUp}
            className="flex h-6 w-6 items-center justify-center rounded text-slate-600 hover:bg-slate-100 disabled:opacity-30"
          >
          ...
          <button
            ...
            className="flex h-6 w-6 items-center justify-center rounded text-slate-600 hover:bg-rose-50 hover:text-rose-700"
          >
            <svg className="h-3.5 w-3.5" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden>
      ...
      <button
        ...
        className="absolute right-2 top-2 flex flex-col items-center gap-[3px] opacity-0 transition-opacity cursor-grab group-hover:opacity-100"
      >
        {[0, 1, 2].map((row) => (
      ...
      <p className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-slate-500">{typeLabel}</p>

      <div className="flex items-start justify-between gap-2.5">
        <div className="min-w-0 flex-1">
          <div className="flex items-start gap-2.5">
            <div className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md ${nodeIconStyle(node.kind)}`}>
              <NodeIcon kind={node.kind} />
            </div>
            <div className="min-w-0">
              <p className="truncate text-[13px] font-semibold leading-5 text-slate-900">{displayNodeTitle(node, palette?.label || node.kind)}</p>
              <p className="mt-0.5 line-clamp-2 text-[11px] leading-4 text-slate-600">{summary}</p>
              {node.note ? <p className="mt-1 line-clamp-2 text-[11px] italic text-slate-500">{node.note}</p> : null}
              {isBranchNode(node) ? (
                <div className="mt-1.5 flex flex-wrap gap-1.5">
                  {node.lanes.slice(0, 4).map((lane) => (
                    <span key={lane.id} className="rounded-full border border-violet-200 bg-violet-50 px-2 py-0.5 text-[10px] font-semibold text-violet-700">
                      {lane.label}
                    </span>
                  ))}
                </div>
        ...
        {node.statusLabel ? (
          <span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[10px] font-medium ${getEngagementStatusChipClass(node.statusLabel)}`}>
            {node.statusLabel}
          </span>
        ) : null}
      </div>
