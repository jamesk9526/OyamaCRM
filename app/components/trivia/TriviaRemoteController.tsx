...
export default function TriviaRemoteController({ eventId }: { eventId: string }) {
  ...
  if (!session) return <main className="min-h-screen bg-[#101010] p-5 text-white"><div className="mx-auto max-w-md border border-[#4b4b4b] bg-[#1f1f1f] p-5"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#9cc5e8]">Oyama Trivia</p><h1 className="mt-1 text-2xl font-semibold">Event-night sign-in</h1><p className="mt-2 text-sm text-slate-300">{message}</p><form onSubmit={claimAccess} className="mt-5 space-y-3"><label className="block text-sm font-semibold">Temporary access code<input value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} autoCapitalize="characters" className="mt-1 h-12 w-full border border-[#8a8886] bg-white px-3 font-mono text-lg tracking-[0.18em] text-slate-950" placeholder="A1B2C3D4" /></label><button disabled={busy || !code.trim()} className="h-12 w-full bg-[#0f6cbd] font-semibold text-white disabled:opacity-50">{busy ? "Connecting…" : "Connect to event"}</button></form></div></main>;
