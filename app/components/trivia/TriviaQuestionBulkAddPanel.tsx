...
export default function TriviaQuestionBulkAddPanel({ rounds, defaultPoints, defaultTimeLimitSec, onAddQuestions }: TriviaQuestionBulkAddPanelProps) {
  ...
  return <section className="trivia-question-entry rounded-2xl border border-indigo-200 bg-white p-4 shadow-sm sm:p-5">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-indigo-100 text-indigo-700"><FilePlus2 className="h-5 w-5" /></span><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-indigo-700">Batch entry</p><h2 className="mt-0.5 text-base font-bold text-slate-950">Have a prepared question list?</h2><p className="mt-0.5 text-sm text-slate-600">Paste multiple questions into a round in one save.</p></div></div><button type="button" onClick={() => setOpen((value) => !value)} disabled={rounds.length === 0} aria-expanded={open} className="event-trivia-secondary-action shrink-0 disabled:cursor-not-allowed disabled:opacity-45"><Plus className="h-4 w-4" />{open ? "Close batch entry" : "Paste questions"}</button></div>
