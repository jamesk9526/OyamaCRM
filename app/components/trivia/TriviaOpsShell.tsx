...
export default function TriviaOpsShell({ children }: TriviaOpsShellProps) {
  ...
  return (
    <div className="min-h-screen h-screen overflow-hidden bg-[#f5f4f8] text-slate-900">
      <header className="relative z-10 flex h-16 items-center justify-between border-b border-[#d9d2e8] bg-white px-4 shadow-[0_1px_2px_rgba(31,20,55,0.08)]">
        ...
        <Link href="/apps" className="border border-[#a99ad0] bg-white px-3 py-1.5 text-xs font-semibold text-[#4a327f] hover:bg-[#f6f2ff]">
          Apps Home
        </Link>
      </header>

      <div className="relative z-10 flex h-[calc(100%-4rem)]">
        <aside className="w-72 shrink-0 overflow-auto border-r border-[#3f2c6e] bg-[#2b2142]">
          <div className="space-y-2 border-b border-[#4b3975] px-3 py-4">
            <p className="text-[10px] uppercase tracking-[0.18em] text-[#d9cffa]">Event command status</p>
            {activeEvent && activeLive ? (
              <div className="space-y-1.5 border border-[#8067b7] bg-[#372957] p-3 shadow-[inset_3px_0_0_#a78bfa]">
              ...
              <div className="border border-[#4b3975] bg-[#372957] p-3">
          ...
          <nav className="space-y-4 p-3">
            {navGroups.map((group) => (
              <section key={group.label}>
                <p className="mb-1 px-1 text-[10px] uppercase tracking-[0.16em] text-[#cbbef0]">{group.label}</p>
          ...
          <div className="mt-auto border-t border-[#4b3975] px-3 py-3">
            <p className="text-xs text-[#cbbef0]">Event workspace · host, check-in, scoring, and projector tools.</p>
          </div>
        </aside>

        <main className="flex-1 overflow-auto bg-[#f5f4f8] p-4 sm:p-6 xl:p-8">{children}</main>
      </div>
    </div>
  );
}
