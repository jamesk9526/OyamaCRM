// Compassion CRM dashboard workspace — shows workflow-first caseload, schedule, tasks, and activity.
...
/** One action row in the dashboard's daily work queue. */
...
/** Builds a single spreadsheet-style triage queue from appointments and follow-ups. */
function buildWorkQueue(data: DashboardData | null): WorkQueueRow[] {
  const appointmentRows = (data?.todaysAppointments ?? []).map((appointment) => ({
    id: `appointment-${appointment.id}`,
    time: formatTime(new Date(appointment.startTime)),
    person: appointment.client ? `${appointment.client.firstName} ${appointment.client.lastName}` : "Unknown client",
    work: appointment.appointmentType.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
    status: "Scheduled today",
    priority: "Time-sensitive",
    href: "/compassion/appointments",
  }));

  const followUpRows = (data?.upcomingFollowUps ?? []).slice(0, 8).map((followUp) => {
    const due = new Date(followUp.dueDate);
    const overdue = followUp.status === "OVERDUE" || due < new Date();
    return {
      id: `follow-up-${followUp.id}`,
      time: overdue ? "Overdue" : due.toLocaleDateString("en-US", { month: "short", day: "numeric" }),
      person: followUp.client ? `${followUp.client.firstName} ${followUp.client.lastName}` : "No client linked",
      work: followUp.title,
      status: followUp.status.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
      priority: followUp.priority.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
      href: "/compassion/follow-ups",
    };
  });

  return [...appointmentRows, ...followUpRows].slice(0, 12);
}

/** A readable daily action table for staff who prefer spreadsheet-style work queues. */
function DailyWorkQueue({ rows }: { rows: WorkQueueRow[] }) {
  return (
    <section className="rounded-lg border border-gray-200 bg-white shadow-sm">
      ...
      {rows.length === 0 ? (
        <div className="px-4 py-8 text-center text-sm text-gray-500">
          No appointments or follow-ups need attention in the current dashboard window.
        </div>
      ) : (
          ...
          <table className="w-full min-w-[820px] border-separate border-spacing-0 text-sm">
            <thead>
              <tr className="bg-gray-50 text-left text-xs font-semibold uppercase text-gray-600">
                <th className="border-b border-gray-200 px-4 py-2">When</th>
                <th className="border-b border-gray-200 px-4 py-2">Client</th>
                <th className="border-b border-gray-200 px-4 py-2">Work Needed</th>
                <th className="border-b border-gray-200 px-4 py-2">Status</th>
                <th className="border-b border-gray-200 px-4 py-2">Priority</th>
                <th className="border-b border-gray-200 px-4 py-2">Open</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row) => (
                <tr key={row.id} className="hover:bg-blue-50/60">
                  <td className="border-b border-gray-100 px-4 py-3 font-mono text-xs text-gray-700">{row.time}</td>
                  <td className="border-b border-gray-100 px-4 py-3 font-medium text-gray-900">{row.person}</td>
                  <td className="border-b border-gray-100 px-4 py-3 text-gray-700">{row.work}</td>
                  <td className="border-b border-gray-100 px-4 py-3 text-gray-600">{row.status}</td>
                  <td className="border-b border-gray-100 px-4 py-3 text-gray-600">{row.priority}</td>
                  <td className="border-b border-gray-100 px-4 py-3">
                    <a href={row.href} className="text-sm font-semibold text-blue-700 hover:underline">Open</a>
                  </td>
                </tr>
              ))}
            </tbody>
 ...
 */
export default function CompassionDashboardWorkspace() {
  const { user } = useAuth();
  ...
  const workQueueRows = buildWorkQueue(data);
  ...
  return (
    <div className="max-w-screen-2xl space-y-4">
      ...
      <WorkspaceBreadcrumbBar
        ...
        metadata={`Updated ${refreshedAt}`}
        accentTone="blue"
        primaryAction={<WorkspaceRibbonButton label="Refresh" onClick={load} accentTone="blue" />}
      />

      <WorkspaceRibbon>
        <WorkspaceRibbonGroup label="Open">
          <WorkspaceRibbonButton label="Clients" href="/compassion/clients" accentTone="blue" />
          <WorkspaceRibbonButton label="Cases" href="/compassion/cases" accentTone="blue" />
          <WorkspaceRibbonButton label="Appointments" href="/compassion/appointments" accentTone="blue" />
          <WorkspaceRibbonButton label="Follow Ups" href="/compassion/follow-ups" accentTone="blue" />
        </WorkspaceRibbonGroup>
        <WorkspaceRibbonGroup label="Review">
          <WorkspaceRibbonButton label="Reports" href="/compassion/reports" accentTone="blue" />
          <WorkspaceRibbonButton label="Data Tools" href="/compassion/data-tools" accentTone="blue" />
        </WorkspaceRibbonGroup>
      </WorkspaceRibbon>

      <section className="rounded-lg border border-blue-100 bg-gradient-to-r from-blue-50 to-white px-4 py-3">
        <h1 className="text-xl font-semibold text-gray-900">
          {getGreeting()}, {userName}.
        </h1>
        <p className="mt-1 max-w-3xl text-sm leading-6 text-gray-600">
          Start with the work queue, then use the metric tiles to spot clients, cases, appointments, and follow-ups that need attention.
        </p>
      </section>
      ...
      {!loading && <DailyWorkQueue rows={workQueueRows} />}
