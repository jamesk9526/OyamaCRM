// Compassion CRM cases workspace — spreadsheet-style case list with create and update workflows.
"use client";
...
const CASE_STATUSES = ["OPEN", "IN_PROGRESS", "PENDING", "CLOSED", "ARCHIVED"] as const;
            ...
            <div>
              ...
              <select value={form.priority} onChange={(e) => set("priority", e.target.value)}
                ...
                {CASE_PRIORITIES.map((priority) => (
                  <option key={priority} value={priority}>{typeLabel(priority)}</option>
                ))}
              </select>
            </div>
          </div>
...
/**
 * EditCaseModal updates case status, type, priority, summary, and staff assignment.
 * It uses the existing PUT /api/compassion/cases/:id endpoint, so no partial UI state is fake.
 */
function EditCaseModal({ caseRecord, onClose, onSaved, staffList }: {
  caseRecord: CompassionCase;
  onClose: () => void;
  onSaved: () => void;
  staffList: StaffUser[];
}) {
  const [form, setForm] = useState({
    ...
    assignedStaffId: caseRecord.assignedStaff?.id ?? "",
    summary: caseRecord.summary ?? "",
  });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const set = (k: string, v: string) => setForm((current) => ({ ...current, [k]: v }));
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      await apiFetch(`/api/compassion/cases/${caseRecord.id}`, {
        method: "PUT",
        body: JSON.stringify({
          ...
          closedAt: form.caseStatus === "CLOSED" ? new Date().toISOString() : null,
        }),
      });
      onSaved();
      onClose();
  ...
  return (
    <WorkspaceSetupModal
      title={`Edit ${caseRecord.caseNumber}`}
      subtitle="Update the case record, then save. Changes are written to the Compassion case API."
      checklist={["1. Review status", "2. Confirm assignment", "3. Save changes"]}
      onClose={onClose}
      maxWidthClassName="max-w-4xl"
    >
      <form onSubmit={handleSubmit} className="px-6 pb-6 pt-14 space-y-4">
        {error && <p className="text-sm text-red-600 bg-red-50 rounded-lg px-3 py-2">{error}</p>}

        <div className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
          <p className="text-xs font-semibold uppercase text-gray-500">Client</p>
          <p className="text-sm font-semibold text-gray-900">
            {caseRecord.client ? `${caseRecord.client.firstName} ${caseRecord.client.lastName}` : "No client linked"}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div>
            ...
            <select value={form.caseStatus} onChange={(e) => set("caseStatus", e.target.value)}
              ...
              {CASE_STATUSES.map((status) => <option key={status} value={status}>{typeLabel(status)}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Case Type</label>
            <select value={form.caseType} onChange={(e) => set("caseType", e.target.value)}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white">
              {CASE_TYPES.map((caseType) => <option key={caseType} value={caseType}>{typeLabel(caseType)}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Priority</label>
            <select value={form.priority} onChange={(e) => set("priority", e.target.value)}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white">
              {CASE_PRIORITIES.map((priority) => <option key={priority} value={priority}>{typeLabel(priority)}</option>)}
            </select>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea rows={4} value={form.summary} onChange={(e) => set("summary", e.target.value)}
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm resize-none"
            placeholder="Case summary, current need, next step, or staff context." />
        </div>
...
export default function CasesWorkspace() {
  ...
  const [editingCase, setEditingCase] = useState<CompassionCase | null>(null);
  ...
  /** Quick case status transitions keep high-volume queues spreadsheet-friendly. */
  async function updateCaseStatus(caseRecord: CompassionCase, nextStatus: string) {
    setError(null);
    try {
      await apiFetch(`/api/compassion/cases/${caseRecord.id}`, {
        method: "PUT",
    ...
    <div className="space-y-6">
      ...
      <WorkspaceRibbon>
        ...
        <WorkspaceRibbonGroup label="Status">
          ...
          <WorkspaceRibbonButton label="Closed" onClick={() => setStatusFilter("CLOSED")} active={statusFilter === "CLOSED"} accentTone="blue" />
        </WorkspaceRibbonGroup>
        ...
        {loading ? (
          ...
          <table className="w-full min-w-[1120px] border-separate border-spacing-0 text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredCases.map((c) => (
                <tr key={c.id} className="hover:bg-blue-50/60 transition-colors align-top">
                  ...
                  <td className="px-4 py-3 text-gray-600">{typeLabel(c.caseType)}</td>
                  ...
                  <td className="px-4 py-3 text-gray-600 max-w-[280px]">
                    <div className="line-clamp-2">{c.summary || "Review case and add a next step."}</div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1.5">
                      <button type="button" onClick={() => setEditingCase(c)}
                      ...
                      {c.caseStatus !== "IN_PROGRESS" && (
                        <button type="button" onClick={() => void updateCaseStatus(c, "IN_PROGRESS")}
                          className="rounded border border-blue-200 px-2 py-1 text-xs font-medium text-blue-700 hover:bg-blue-50">
                          Start
                        </button>
                      )}
                      {c.caseStatus !== "CLOSED" && (
                        <button type="button" onClick={() => void updateCaseStatus(c, "CLOSED")}
