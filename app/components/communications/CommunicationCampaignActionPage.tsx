...
export default function CommunicationCampaignActionPage({ title, helper, ctaLabel, ctaHref }: CommunicationCampaignActionPageProps) {
  ...
  const canonicalHref = campaignId
    ? ctaHref
  ...
  return (
    <section className="space-y-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Link href={canonicalHref} className="rounded-lg border border-green-600 bg-green-600 px-3 py-2 text-xs font-semibold text-white hover:bg-green-700">
          {ctaLabel}
        </Link>
      </div>
    </section>
