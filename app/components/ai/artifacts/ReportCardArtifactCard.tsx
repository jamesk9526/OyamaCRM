    ...
    <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white text-slate-700 shadow-[0_18px_40px_rgba(15,23,42,0.08)]">
      ...
      <header className="flex items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3">
        <div className="flex items-center gap-2">
          {/* Bar chart icon */}
          <svg className="h-4 w-4 shrink-0 text-emerald-600" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M3 3v18h18" />
            <path strokeLinecap="round" strokeLinejoin="round" d="M8 16V8m4 8v-5m4 5v-3" />
          </svg>
          <h4 className="text-sm font-semibold text-slate-950">{title || "CRM Report"}</h4>
        </div>
        ...
        <div className="grid grid-cols-2 gap-px border-b border-slate-200 bg-slate-200">
          {metrics.map((m, i) => (
            <div key={i} className="bg-white px-4 py-3">
              ...
              <p className="mt-1 text-lg font-semibold leading-tight text-slate-950 tabular-nums">{m.value}</p>
              {(m.delta || m.trend) && (
                <div className="mt-0.5 flex items-center gap-1">
                  <TrendIcon trend={m.trend} />
                  {m.delta && (
                    <span className={`text-[10px] font-medium ${m.trend === "up" ? "text-emerald-600" : m.trend === "down" ? "text-rose-600" : "text-slate-500"}`}>
                      {m.delta}
                    </span>
                  )}
                </div>
        ...
        <div className="flex items-center justify-end px-4 py-3">
          {onOpenReport ? (
            <button
              ...
              className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 shadow-sm transition-colors hover:bg-slate-50"
            >
      ...
      {onAskReportQuestion && (
        <div className="border-t border-slate-200 px-4 py-3">
          <p className="mb-2 text-[10px] font-medium uppercase tracking-wide text-emerald-700">Report tools</p>
          <div className="mb-2 flex flex-wrap gap-1.5">
            {quickPrompts.map((prompt) => (
              <button
                ...
                className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 text-[11px] text-slate-700 hover:bg-white"
              >
                {prompt}
              </button>
            ))}
