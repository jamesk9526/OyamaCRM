...
export default function HostedGivingPage({ token }: { token: string }) {
  ...
  if (loading) return <main className="flex min-h-screen items-center justify-center bg-slate-100 p-6"><div className="size-9 animate-spin rounded-full border-4 border-slate-200 border-t-blue-600" aria-label="Loading giving page" /></main>;
  if (loadError || !payload) return <main className="flex min-h-screen items-center justify-center bg-slate-100 p-6"><section className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm"><Heart className="mx-auto size-9 text-slate-400" /><h1 className="mt-4 text-xl font-semibold tracking-tight text-slate-950">Giving page unavailable</h1><p className="mt-2 text-sm leading-6 text-slate-600">{loadError || "This page may be unpublished or its link may have changed."}</p></section></main>;

  const fieldClass = "mt-2 min-h-12 w-full rounded-lg border border-slate-300 bg-white px-3.5 text-base text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 hover:border-slate-400 focus:border-blue-600 focus:ring-4 focus:ring-blue-100";
  const sectionClass = "border-t border-slate-200 pt-7";
  const pageBackground = payload.appearance.themeMode === "soft" ? payload.appearance.backgroundColor : "#f1f5f9";

  return (
    <main className="min-h-screen px-3 py-3 text-slate-900 sm:px-6 sm:py-8 lg:px-8" style={{ backgroundColor: pageBackground }}>
      <div className="mx-auto grid w-full max-w-6xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.12)] lg:grid-cols-[minmax(280px,0.78fr)_minmax(0,1.22fr)]">
        <aside className="relative flex flex-col justify-between gap-10 overflow-hidden bg-slate-950 p-6 text-white sm:p-9 lg:min-h-[720px] lg:p-11">
          <div className="pointer-events-none absolute -right-24 -top-24 size-72 rounded-full opacity-20 blur-3xl" style={{ backgroundColor: accent }} />
          <div>
            <div className="relative inline-flex size-12 items-center justify-center rounded-2xl ring-1 ring-white/10" style={{ backgroundColor: `${accent}33`, color: accent }}><Heart className="size-6" fill="currentColor" /></div>
            <p className="mt-9 text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">{payload.form.hostedPageEyebrow}</p>
            <h1 className="mt-4 max-w-xl text-3xl font-semibold leading-[1.08] tracking-tight sm:text-4xl lg:text-[2.75rem]">{payload.form.headline}</h1>
            <p className="mt-5 max-w-md text-sm leading-7 text-slate-300 sm:text-base">{payload.form.supportingCopy}</p>
            <div className="mt-8 flex items-center gap-3 text-sm font-medium text-slate-200"><span className="inline-flex size-8 items-center justify-center rounded-full bg-white/10" style={{ color: accent }}><Heart className="size-4" fill="currentColor" /></span><span>Your generosity creates lasting change.</span></div>
          </div>
          <div className="relative space-y-4 border-t border-white/15 pt-6 text-sm text-slate-300">
            ...
            <p className="leading-6">{payload.form.hostedPageFooter}</p>
            <p className="text-xs text-slate-400">Hosted by OyamaCRM for {payload.organizationName || payload.siteName}</p>
          </div>
        </aside>

        <section className="min-w-0 p-5 sm:p-9 lg:p-11">
          {payload.stripeMode === "sandbox" ? <div className="mb-6 rounded-lg border border-amber-300 bg-amber-50 px-3.5 py-3 text-sm font-semibold text-amber-900">Test mode — no live payment will be charged.</div> : null}
          {completed ? (
            ...
            <form onSubmit={submit} noValidate={false}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div><h2 className="text-2xl font-semibold tracking-tight text-slate-950">Your gift</h2><p className="mt-1.5 text-sm leading-6 text-slate-600">Choose an amount and tell us who you are.</p></div>
                <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1.5 text-xs font-semibold text-emerald-800"><LockKeyhole className="size-3.5" />Secure form</span>
              </div>

              <fieldset className="mt-8"><legend className="text-sm font-semibold text-slate-900">Gift amount</legend><div className="mt-3 grid grid-cols-2 gap-2.5 sm:grid-cols-3">{payload.form.suggestedAmounts.slice(0, 8).map((preset) => { const active = !customAmount && selectedAmount === preset; return <button key={preset} type="button" aria-pressed={active} onClick={() => { setSelectedAmount(preset); setCustomAmount(""); }} className={`min-h-12 rounded-lg border px-3 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-100 ${active ? "text-white shadow-sm" : "border-slate-300 bg-white text-slate-800 hover:border-slate-400 hover:bg-slate-50"}`} style={active ? { backgroundColor: accent, borderColor: accent } : undefined}>{money(preset, payload.currency)}</button>; })}{payload.form.allowCustomAmount ? <button type="button" aria-pressed={selectedAmount === null} onClick={() => { setSelectedAmount(null); setCustomAmount((current) => current || ""); }} className={`min-h-12 rounded-lg border px-3 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-100 ${selectedAmount === null ? "border-blue-400 bg-blue-50 text-blue-800" : "border-slate-300 bg-white text-slate-800 hover:border-slate-400 hover:bg-slate-50"}`}>{payload.form.customAmountLabel}</button> : null}</div></fieldset>
              {payload.form.allowCustomAmount && selectedAmount === null ? <label className="mt-3 block text-sm font-semibold text-slate-800" htmlFor="custom-gift-amount">Custom amount<input id="custom-gift-amount" name="customAmount" type="number" inputMode="decimal" min={payload.form.minimumAmountCents / 100} max={payload.form.maximumAmountCents / 100} step="0.01" required className={fieldClass} value={customAmount} onChange={(event) => setCustomAmount(event.target.value)} /></label> : null}

              {payload.form.enableMonthlyGiving ? <fieldset className={sectionClass}><legend className="text-sm font-semibold text-slate-900">Giving schedule</legend><div className="mt-3 grid gap-2.5 sm:grid-cols-2">{(["one-time", "monthly"] as const).map((value) => <label key={value} className={`flex min-h-12 cursor-pointer items-center gap-3 rounded-lg border px-3.5 text-sm font-semibold transition ${giftType === value ? "border-blue-500 bg-blue-50 text-blue-900 shadow-sm" : "border-slate-300 text-slate-700 hover:border-slate-400"}`}><input type="radio" name="giftType" value={value} checked={giftType === value} onChange={() => setGiftType(value)} className="size-4 accent-blue-600" />{value === "monthly" ? "Monthly gift" : "One-time gift"}</label>)}</div></fieldset> : null}

              {payload.form.allowedDesignations.length > 0 ? <label className={sectionClass + " block text-sm font-semibold text-slate-800"} htmlFor="gift-designation">Gift designation<select id="gift-designation" name="designation" value={designation} onChange={(event) => setDesignation(event.target.value)} className={fieldClass}>{payload.form.allowedDesignations.map((item) => <option key={item} value={item}>{item}</option>)}</select></label> : null}

              <fieldset className={sectionClass}><legend className="text-sm font-semibold text-slate-900">Donor information</legend><div className="mt-3 grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium text-slate-700" htmlFor="donor-first-name">First name<input id="donor-first-name" name="firstName" autoComplete="given-name" required={payload.form.requireDonorName} maxLength={80} className={fieldClass} /></label><label className="text-sm font-medium text-slate-700" htmlFor="donor-last-name">Last name<input id="donor-last-name" name="lastName" autoComplete="family-name" required={payload.form.requireDonorName} maxLength={80} className={fieldClass} /></label><label className="text-sm font-medium text-slate-700 sm:col-span-2" htmlFor="donor-email">Email address<input id="donor-email" name="email" type="email" autoComplete="email" required maxLength={254} className={fieldClass} /></label>{payload.form.collectPhone ? <label className="text-sm font-medium text-slate-700 sm:col-span-2" htmlFor="donor-phone">Phone <span className="font-normal text-slate-500">(optional)</span><input id="donor-phone" name="phone" type="tel" autoComplete="tel" maxLength={40} className={fieldClass} /></label> : null}</div></fieldset>
