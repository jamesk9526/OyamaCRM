  ...
  return (
    <button
      ...
      className={[
        ...
        active ? "border-violet-300 bg-violet-50" : "border-slate-200 bg-white hover:border-violet-200 hover:bg-violet-50/50",
        !href ? "opacity-85" : "",
      ].join(" ")}
    >
    ...
    <div className="space-y-3">
      ...
      <WorkspaceRibbon>
        ...
        <WorkspaceRibbonGroup label="Selected Event">
          ...
          <WorkspaceRibbonButton label="Check-In" href={selectedEventId ? `/events/${selectedEventId}/check-in` : undefined} disabled={!selectedEventId} accentTone="purple" variant="primary" />
        </WorkspaceRibbonGroup>
      </WorkspaceRibbon>
      ...
      <section className="grid gap-3 xl:grid-cols-[minmax(0,1.35fr)_minmax(360px,0.65fr)]">
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <label className="w-full max-w-md space-y-1">
              ...
              <select
                ...
                className="h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900 shadow-sm focus:border-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-100"
              >
        ...
        <aside className="rounded-xl border border-violet-200 bg-violet-50 p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wide text-violet-800">Open workspace</p>
          ...
          <button
            ...
            disabled={!targetHref}
            className="mt-4 h-10 w-full rounded-lg bg-violet-600 px-3 text-sm font-semibold text-white hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {targetHref ? "Open Selected Tool" : "Tool Not Ready"}
          </button>
          {targetHref ? (
            <p className="mt-2 break-all text-xs text-violet-900">{targetHref}</p>
          ) : (
            <p className="mt-2 text-xs text-violet-900">This module is listed for planning clarity but is not exposed as a finished tool yet.</p>
          )}
        </aside>
      </section>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <Link href="/events/events" className="text-sm font-semibold text-violet-700 hover:text-violet-900">
            Review all events
          </Link>
        </div>
        <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
          {GLOBAL_EVENTS_TOOLS.map((tool) => (
            <Link
              key={tool.href}
              href={tool.href}
              className="rounded-lg border border-slate-200 px-3 py-2 transition-colors hover:border-violet-300 hover:bg-violet-50/60"
            >
              <p className="text-sm font-semibold text-slate-900">{tool.label}</p>
