...
export default function NewEventModal({ onClose, onCreated }: { onClose: () => void; onCreated: (event: EventItem) => void }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (saving || !form.name.trim() || !form.date || !form.time) return;
    setSaving(true);
    setError(null);
    try {
      const created = await apiFetch<EventItem>("/api/events", { method: "POST", body: JSON.stringify({ name: form.name.trim(), type: form.mode === "TRIVIA" ? "TRIVIA" : "OTHER", mode: form.mode, location: form.location.trim() || null, startDate: new Date(`${form.date}T${form.time}`).toISOString(), active: true }) });
  ...
  return (
    <div className="fixed inset-0 z-[80] grid place-items-center overflow-y-auto bg-slate-950/35 p-3 sm:p-6" role="dialog" aria-modal="true" aria-labelledby="new-event-title">
      ...
      <form ref={dialogRef} tabIndex={-1} onSubmit={submit} className="relative my-auto w-full max-w-xl overflow-hidden rounded-2xl bg-white shadow-2xl">
