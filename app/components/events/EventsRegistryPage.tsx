...
export default function EventsRegistryPage() {
  ...
  const now = Date.now();
  const current = filtered.filter((event) => eventRegistryGroup(event, now) === "current");
  const unscheduled = filtered.filter((event) => eventRegistryGroup(event, now) === "unscheduled");
  const upcoming = filtered.filter((event) => eventRegistryGroup(event, now) === "upcoming").sort((a, b) => new Date(a.startDate).getTime() - new Date(b.startDate).getTime());
