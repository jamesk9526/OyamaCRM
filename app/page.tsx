    ...
    <section className="crm-card-surface mt-5 min-w-0 rounded-xl border p-4 sm:p-5">
      <div className="mb-4 flex items-start justify-between gap-3 px-1 sm:items-center">
        <div className="min-w-0">
          <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">Custom workspace</p>
          <h2 className="mt-1 text-xl font-semibold tracking-tight text-slate-900">At a glance</h2>
          <p className="mt-1 text-xs text-slate-500">Choose the live donor signals that matter to your team, then drag them into the order you use.</p>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <button type="button" onClick={dashboardState.toggleLayoutLock} className="hidden min-h-8 items-center rounded-[2px] px-2 text-[11px] font-semibold text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-800 sm:inline-flex" title={dashboardState.locked ? "Unlock dashboard layout" : "Lock dashboard layout"}>{dashboardState.locked ? "Layout locked" : "Lock layout"}</button>
          <button type="button" onClick={dashboardState.toggleEditMode} disabled={dashboardState.locked} className={`inline-flex min-h-8 items-center rounded-[2px] border px-2.5 text-[11px] font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${dashboardState.editMode ? "border-[#0f6cbd] bg-[#eff6fc] text-[#0f548c]" : "border-slate-200 bg-white text-slate-700 hover:border-[#0f6cbd]"}`}>
            {dashboardState.editMode ? "Done arranging" : "Reorder"}
          </button>
          <span className="hidden items-center gap-1.5 rounded-lg bg-slate-50 px-3 py-1.5 text-[11px] font-semibold text-slate-600 ring-1 ring-slate-200 sm:inline-flex">
