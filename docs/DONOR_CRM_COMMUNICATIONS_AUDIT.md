...
|---|---|---|---|---|
| Communications Campaign Library | Working | API-backed | app/communications/page.tsx, app/communications/library/campaigns/page.tsx, app/components/communications/CampaignWorkspace.tsx, server/src/routes/email-campaigns.ts | `/communications` opens directly to the campaign library, and `/communications/library/campaigns` deep-links into the same canonical view. Campaign CRUD, project preview cards, embedded build workspace, scheduling, send, share/private, readiness, and delivery events are wired. Segment/log depth remains a broader communications follow-up. |
