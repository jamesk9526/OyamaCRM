...
The hosted page reads and submits through `/api/site-embeds/public/*`, which is mounted by the Express API server. Do not route it through a Next-only `/api/giving/*` handler: production reverse proxies normally send all `/api/*` traffic directly to Express.
...
For public giving, Stripe Giving can optionally save an **After payment, redirect to** HTTPS URL. Stripe returns through an OyamaCRM-controlled completion bridge, which then sends the donor to that saved URL. The destination is never accepted from the browser request, preventing the checkout return path from being used as an open redirect.
