...
6. In the builder, use `Recipient Preview` to open the server-rendered donor preview. The same preview contains visual, responsive-width, and plain-text output. After saving, open Publish, resolve every required compliance check, use `Send Test Email` to send a proof only to the supplied reviewer address, then choose `Mark Ready`. Audience review and live sending remain in the campaign workflow.
