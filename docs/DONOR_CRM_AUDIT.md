...
| Reports | Partially Working | [app/reports/page.tsx](app/reports/page.tsx), [app/components/reports-app/ReportsApp.tsx](app/components/reports-app/ReportsApp.tsx), [server/src/routes/reports.ts](server/src/routes/reports.ts), [tests/smoke/reports-smoke.test.ts](tests/smoke/reports-smoke.test.ts) | New Oyama Reports app is canonical and uses live CRM APIs only. Core donation, campaign, giving, lapsed, top-donor, monthly, and board reports run; letter-list persistence, server PDF export, event-scoped donor joins, and some custom builder persistence remain partial or coming soon. | Finish report-specific backends before marking all cards Working; keep card statuses visible as Working, Partial, or Coming Soon. |
