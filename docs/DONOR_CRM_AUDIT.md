...
| Steward Signals | Partially Working | [app/steward-signals/page.tsx](app/steward-signals/page.tsx), [server/src/routes/steward-signals.ts](server/src/routes/steward-signals.ts) | Signals can be over-trusted if not framed as suggestions. | Keep suggestion language explicit and require human review actions. |
