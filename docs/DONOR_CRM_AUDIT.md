...
| Donations list/entry | Working | [app/donations/page.tsx](app/donations/page.tsx), [app/donations/new/page.tsx](app/donations/new/page.tsx), [server/src/routes/donations.ts](server/src/routes/donations.ts) | Acknowledgment workflow is distributed across fields and linked tools; one route-level temporary handoff still uses a browser `window.confirm` instead of in-app modal UX. | Continue unifying thank-you status visibility using existing fields and remove the remaining browser confirm flow. |
...
| Communications Campaign Library | Working | [app/communications/page.tsx](app/communications/page.tsx), [app/communications/library/campaigns/page.tsx](app/communications/library/campaigns/page.tsx), [server/src/routes/email-campaigns.ts](server/src/routes/email-campaigns.ts) | Campaign library create/build/open/send, clone-from-template, share/private, readiness, delivery webhooks, and project-library deep links are wired to real APIs. Provider-specific delivery depth remains a communications analytics follow-up, not a Campaign Library blocker. | Continue expanding provider webhook mappings and unsubscribe event propagation. |
| Letters and Printables | Working | [app/letters-printables/page.tsx](app/letters-printables/page.tsx), [server/src/routes/letters.ts](server/src/routes/letters.ts), [server/src/services/letters-execution.ts](server/src/services/letters-execution.ts), [tests/unit/letters-pdf-layout.test.ts](tests/unit/letters-pdf-layout.test.ts), [tests/smoke/letter-builder-ui-source.test.ts](tests/smoke/letter-builder-ui-source.test.ts) | Server PDF export and batch generation are live, with source-contract and PDF layout tests covering key formatting and generation behavior. | Continue hardening provider-specific delivery and queue operations while preserving current API contracts. |
...
| Steward Paths (visual builder + sequence) | Working | [app/steward-paths/page.tsx](app/steward-paths/page.tsx), [app/steward-paths/builder/page.tsx](app/steward-paths/builder/page.tsx), [app/components/steward-paths/StewardPathsWorkspacePage.tsx](app/components/steward-paths/StewardPathsWorkspacePage.tsx), [server/src/routes/steward-paths.ts](server/src/routes/steward-paths.ts), [tests/api/steward-paths.api.test.ts](tests/api/steward-paths.api.test.ts), [tests/unit/steward-paths-workflow-builder.test.ts](tests/unit/steward-paths-workflow-builder.test.ts) | Canonical workspace, visual builder, branch/status execution, template share/duplicate/test-run/history, and safe draft-first email behavior are implemented. Long-window analytics depth is a future enhancement, not a current workflow blocker. | Keep draft-first email rules and add deeper operations trend dashboards as follow-up. |
...
| Volunteers | Partially Working | [app/volunteers/page.tsx](app/volunteers/page.tsx) | Workspace now uses shared `apiFetch`; deeper volunteer-specific workflow/reporting depth remains limited. | Expand volunteer assignment, hours, and reporting workflows while keeping shared fetch/session behavior. |
| Reports | Working | [app/reports/page.tsx](app/reports/page.tsx), [app/components/donor-reports/DonorReportsSpreadsheet.tsx](app/components/donor-reports/DonorReportsSpreadsheet.tsx), [server/src/routes/reports.ts](server/src/routes/reports.ts), [tests/smoke/reports-smoke.test.ts](tests/smoke/reports-smoke.test.ts) | Reports now live in the main Donor CRM shell as a focused, spreadsheet-style workbook. Month-to-date giving and donor-by-designation sheets use completed gifts from the first of the month through now; CSV exports are permission-gated and printing opens a focused sheet. | Add campaign, retention, and custom date-range worksheets only with matching live-data and server-authorized export support. |
| Data Tools | Working | [app/data-tools/page.tsx](app/data-tools/page.tsx), [app/data-tools/import/page.tsx](app/data-tools/import/page.tsx), [server/src/routes/constituents.ts](server/src/routes/constituents.ts) | Merge operations persist selected values, import rollback/history are live, recent import runs are visible on the main workspace, and full CSV exports now cover core donor entities. | Expand centralized quality checks and admin-grade export tooling as follow-up depth work. |
...
Phase 2 artifact reference (2026-06-09): [docs/status/audit-artifacts/2026-06-09-donor-crm-phase2-audit.md](docs/status/audit-artifacts/2026-06-09-donor-crm-phase2-audit.md)
