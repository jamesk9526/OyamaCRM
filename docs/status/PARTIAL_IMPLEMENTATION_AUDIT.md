...
| PI-013 | Letters PDF export API | server/src/routes/letters.ts | Partially Working | `POST /api/letters/generated/:id/export-pdf` logs partial implementation and returns HTTP 501. | Finish | Wire server PDF generation pipeline and storage/streaming path | Update letters workspace to reflect live export status/errors | API tests for PDF export success/failure and permissions | Open item: backend intentionally returns 501 until pipeline is implemented. |
