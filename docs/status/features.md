...
| Donor report library | Working | `app/reports/page.tsx`, `app/components/donor-reports/DonorReportsSpreadsheet.tsx`, `server/src/services/donor-report-library.ts`, `server/src/routes/reports.ts` | Reports live inside the normal Donor CRM shell as a card library and live workbook. The library includes gifts, payment and designation performance, lifetime and monthly giving, donor files, follow-up, notes, first-time/lapsed/never-given donors, top donors, batch receipt register, an unacknowledged-gifts review queue, and a printable/exportable three-year active/new/repeat donor analysis matrix. A visual chart mode and a selected-period CRM performance scorecard with transparent comparison insights are available. Every visible report has a live API and formula-safe CSV export; unsupported capacity values are explicitly shown only when stored as donor tags. |
