...
| Area | Status | Evidence | Notes |
|---|---|---|---|
| Template publish preflight + confirm endpoint | Working | `server/src/routes/letters.ts`, `app/components/letters/OyamaLettersWorkspace.tsx` | `POST /api/letters/templates/:id/publish` checks merge fields, required header/footer presets, optional selected-signature active state, sample merge validation, and server PDF parser parity metrics before explicit `confirm=true` execution. One-click `POST /api/letters/templates/:id/sample-pdf` provides the server-rendered sample preview. |
...
| Canvas inspector preflight utility | Working | `app/components/letters/OyamaLettersWorkspace.tsx` | The Preflight Checklist card provides live draft checks for name, body, required header/footer presets, and unknown merge fields, plus server preflight summary and `Refresh` / `Save + Refresh` actions. Signature selection is optional. |
