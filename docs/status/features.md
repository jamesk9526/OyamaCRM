...
_Last focused audit: 2026-07-28 (DonorCRM data intake and communications builders)_

## 2026-07-22 DonorCRM v1.3 Enterprise Shell

| Capability | Status | Evidence | Notes |
|---|---|---|---|
| Unified Microsoft-style CRM chrome | Working | `app/globals.css`, `app/components/layout/AppShell.tsx`, `app/components/layout/TopBar.tsx`, `app/components/layout/DonorMegaMenu.tsx`, `app/components/layout/AppProductShell.tsx`, `app/compassion/layout.tsx`, `app/components/events/EventsShell.tsx`, `app/components/trivia/TriviaOpsShell.tsx` | Donor CRM, Compassion CRM, Events, Letters, Email, Trivia administration, and standalone app shells now share Segoe typography, neutral work canvases, flat bordered surfaces, compact controls, and Fluent blue focus/primary actions. Donor keeps the requested dark graphite top/side bars; Trivia retains a purple product identity while legacy dark admin panels normalize to the light operational surface. Projector and public/remote presentation routes remain purpose-specific. |
...
| Reports export, print, and saved-view refresh | Working | `app/components/reports-app/ReportsApp.tsx`, `app/components/reports-app/ReportResultsWorkspace.tsx`, `app/components/reports-app/ReportCategoryRail.tsx` | The DonorCRM reporting hub uses a compact Microsoft-style rail, saves filter views in browser storage, exports the currently loaded live grid to CSV, and prints a streamlined report view. Unsupported letter-list placeholder actions were removed. |
...
| Import center and reusable mappings | Working | `app/data-tools/import/page.tsx`, `app/components/data-tools/GuidedImportWizard.tsx`, `app/data-tools/import/ImportWizard.tsx` | The import center now exposes constituent, donation, event guest, and duplicate-resolution paths together. Constituent field mappings are safely remembered only for the same CSV header set in browser storage; staff can reset to the current smart suggestions before validation. Existing server-side duplicate handling, dry runs, audit history, and rollback remain the import safeguards. |
...
| Trivia event-night temporary access | Working | `server/src/routes/trivia.ts`, `app/components/trivia/TemporaryEventAccessPanel.tsx`, `app/components/trivia/TriviaRemoteController.tsx` | A producer can create short-lived, revocable codes for host and check-in roles. Remote users claim an event-scoped session without CRM credentials; actions are role-checked, expire with the pass, and are added to the Trivia audit trail. Codes are returned only at creation and not persisted in API list responses. |
...
| Trivia visual builder and question media | Working | `app/components/trivia/RoundQuestionBuilderPanel.tsx`, `app/components/trivia/TriviaGameMap.tsx`, `app/apps/trivia/hooks/useTriviaModuleState.ts`, `app/components/trivia/ProjectorDisplayView.tsx`, `server/src/routes/trivia.ts` | The visual game canvas is now an authoring surface: producers can add rounds and questions from the map, drag rounds into run order, reorder questions, and move questions between rounds. Arrow controls provide an accessible non-drag fallback, and every move persists to the same local/server-synced event state used by host and projector views. Round and question creation use focused modals. Image, audio, and video questions support direct links or authenticated uploads (images up to 5MB; supported audio/video up to 12MB) with builder previews and projector rendering. |
