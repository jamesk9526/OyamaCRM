...
| Organized drag-and-drop email builder | Working | `app/components/oyama-email/OyamaEmailBuilderWorkspace.tsx`, `tests/smoke/oyama-email-workspace-source.test.ts` | The compact two-row builder bar keeps workflow, readiness, zoom, and email-chrome guidance available without consuming canvas height. The flat block list is now a collapsible Block Library with Content, Layout, and Engagement drawers. A separate, initially closed Merge Field Drawer keeps CRM data categories out of the authoring flow until needed. Blocks can be clicked or dragged into explicit canvas insertion lines; Layout also groups ready-made column and reusable-section templates. Builder surfaces now use an airy indigo/blue palette. |
