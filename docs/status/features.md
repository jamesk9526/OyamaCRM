...
| Saved base lists can be edited directly | Working | `app/components/contacts-manager/AudienceListManager.tsx`, `ContactsManagerPage.tsx`, `PUT /api/email-campaigns/lists/:listId`, `DELETE /api/email-campaigns/lists/:listId/recipients/:memberId` | The Fluent-style Audience List Builder uses one base-list selector, visible saved/unsaved state, labeled member controls, and an explicit save command. Full membership replacement is transactional, so future email, letter, label, and export workflows see either the complete prior list or the complete revised list. |
| Audience list removal tools and filters | Working | `app/components/contacts-manager/AudienceListManager.tsx`, `POST /api/email-campaigns/lists/:listId/recipients/remove` | Each member has a removal checkbox, with select-all-shown, remove-selected, and confirmed remove-all actions. Search combines with Include Church tags / Exclude Church tags filters; closed accounts are always excluded. |
| Segment Builder direct list removal | Working | `app/components/contacts-manager/ContactsManagerPage.tsx`, `DELETE /api/email-campaigns/lists/:listId/recipients/:memberId`, `POST /api/email-campaigns/lists/:listId/recipients/remove` | Every constituent row and the member inspector expose a labeled Remove from list action. Saved-list row removals commit immediately; the filter bar supports Include/Exclude Church tags, and Remove filtered applies the current search/tag/type filters to the selected base list after confirmation. |
| System-wide constituent account closure | Working after migration | `prisma/migrations/20260813170000_add_constituent_account_closure`, `server/src/lib/prisma.ts`, `POST /api/constituents/:id/close`, `app/constituents/[id]/page.tsx` | Closing retains history and an audited reason while making the constituent invisible to shared operational queries. It enforces all contact restrictions, removes saved-list memberships, archives tasks, cancels active stewardship paths, suppresses subscriptions, and makes queued unsent email recipients ineligible. |
| Browser session and background-service reliability | Working | `app/lib/auth-client.ts`, `AuthProvider.tsx`, `PluginProvider.tsx`, `TopBar.tsx`, `server/src/routes/auth.ts` | Browser API calls use the first-party `/api` proxy so refresh cookies do not depend on third-party-cookie policy. QuickBooks status waits for auth restoration, notification SSE uses an authenticated fetch stream, signed-out refresh probes return 204, and concurrent-tab refresh rotation retries once without clearing another tab's new cookie. |
| Stale branding media and responsive chart recovery | Working | `server/src/lib/branding-assets.ts`, `server/src/routes/settings.ts`, `server/src/services/organization-branding.ts`, `DonorReportsSpreadsheet.tsx` | Missing local branding uploads are removed from returned branding context so existing text/initial fallbacks render without repeated broken-image requests. Recharts containers declare usable minimum width/height constraints for narrow, hidden, and resized layouts. |
...
| Capability | Status | Evidence | Notes |
|---|---|---|---|
| Focused email composition workspace | Working | `app/components/oyama-email/OyamaEmailBuilderWorkspace.tsx`, `tests/smoke/oyama-email-workspace-source.test.ts` | The always-open block rail and duplicate advanced editor are removed from the active workflow. A single `+ Add block` action opens a grouped, viewport-safe block modal; selecting a canvas block opens its only editing surface in the right inspector. Email settings remain available as the second inspector tab. |
| Global branding authority | Working | `app/components/oyama-email/OyamaEmailBuilderWorkspace.tsx`, `server/src/services/oyama-email/email-render-service.ts`, `tests/unit/oyama-email-render-service.test.ts` | Organization font, primary/link color, email width, canvas background, managed header/footer, organization name, and physical address come from Branding Settings in builder preview, recipient preview, proof, and send rendering. Legacy template-local branding values are accepted during upgrade but cannot override the global source. |
| Email block and file delivery | Working | `app/components/oyama-email/OyamaEmailBuilderWorkspace.tsx`, `server/src/routes/email-campaigns.ts`, `tests/api/public-email-media.api.test.ts` | Existing structured/nested blocks remain supported. Image upload keeps signature validation; File Link blocks can upload validated PDF, DOCX, XLSX, CSV, and TXT documents up to 10 MB at top level or inside nested columns and serve them from the scoped media path. |
| Compatibility conversion | Working | `server/src/routes/oyama-email.ts`, `server/src/services/oyama-email/email-render-service.ts`, `tests/unit/oyama-email-render-service.test.ts`, `tests/unit/oyama-email-letter-conversion.test.ts` | Raw, wrapped, malformed, and HTML-only legacy template payloads continue to recover into the canonical block document. Legacy block families normalize through the production renderer, and email↔letter round-trip metadata preserves the original structured email without sending or publishing. |

Validation: focused renderer, conversion, compliance, media, and builder source suites pass 35/35; server typecheck passes; focused ESLint has zero errors. Authenticated browser proof and database-backed send/API suites remain blocked by the unavailable local MySQL service at `localhost:3306`.

## 2026-08-12 Letter Preview and Donor Reporting Pass

| Capability | Status | Evidence | Notes |
|---|---|---|---|
| Pre-generation letter downloads | Working | `app/components/letters/OyamaLettersWorkspace.tsx`, `POST /api/letters/generated/preview-pdf-batch` | Preview exposes the current PDF, ZIP files containing one production PDF per included recipient, and combined batch PDFs. Current-PDF saves use a native download link. Large ZIP/batch exports are prepared sequentially in 40-recipient, gateway-safe parts with visible progress and a native direct-save link for every completed part; server merge and PDF work also uses bounded concurrency. After every batch-PDF part is ready, staff can merge the parts locally in the browser and download one master PDF without another gateway-bound generation request. These paths do not create generated-letter records. |
...
| Avery 5160 mail merge | Working | `app/components/letters/MailMergeLabelsWorkspace.tsx`, `server/src/services/avery-labels.ts`, `POST /api/letters/labels/avery-5160.pdf` | OyamaLetters has a dedicated 30-up label workspace with Contacts Manager audience lists, recipient review, server-enforced mail suppression and complete addresses, partially used sheet offsets, alignment guides, multi-sheet PDF output, and audit logging. |
...
| Phone-first public registration | Working | `EventPageBuilderPreview.tsx`, `PublicEventRegistrationForm.tsx`, `tests/unit/event-public-mobile-contract.test.ts` | Public sections reflow at narrow widths, long ticket labels and receipts wrap safely, controls use mobile touch targets, safe-area padding is honored, and the floating registration action hides while the form is in view. |
...
| Capability | Status | Evidence | Notes |
|---|---|---|---|
| Review-first Donor Research workspace | Working | `app/donor-research/page.tsx`, `app/components/donor-research/DonorResearchWorkspace.tsx`, `server/src/routes/donor-research.ts`, `server/src/services/public-donor-research.ts` | Staff can select a constituent; run transient ProPublica, configured SEC, or licensed WealthEngine individual lookups; document identity-match confidence/rationale; save an unverified finding; and explicitly verify or dismiss it. Individual screening requires an authorization confirmation and constituent edit permission. This is external evidence research, not the internal cohort analysis in Steward Signals. |
...
| External research adapters | Partially Working | ProPublica, SEC, and WealthEngine adapters plus `.env.example` | Nonprofit Explorer is available without a local key. SEC EDGAR requires `SEC_EDGAR_USER_AGENT`. Individual screening requires a licensed `WEALTHENGINE_API_KEY`; the sandbox is synthetic and cannot create findings. IRS bulk data remains a future adapter. |
...
Validation: the original focused donor/report/research coverage passed 134/134, and the individual-screening follow-up passed its 10/10 adapter/wiring tests; focused ESLint and web/server typechecks passed; Prisma validation/client generation passed; the production build passed with 186 routes. Browser checks covered the original Donor Research desktop/phone layouts and confirmed the individual-screening build still enforces the unauthenticated redirect. Database migration deployment remains blocked by the pre-existing local-history drift documented in the audit.
...
## 2026-08-11 OyamaCRM v1.45b CRM Release
...
| Donor amount precision | Working | `prisma/schema.prisma`, `app/components/donations/donation-utils.ts`, `app/components/constituents/constituent-utils.ts`, `app/components/dashboard/*` | Donation amounts are stored as `Decimal(10,2)` and donor-facing lists, payments, profiles, dashboards, and campaign cards now render two decimal places. This prevents presentation-layer whole-dollar rounding from obscuring gift amounts during reconciliation. |
| Dashboard/report calculation consistency | Working | `server/src/lib/dateRanges.ts`, `server/src/lib/money.ts`, `server/src/routes/reports.ts`, `server/src/routes/donations.ts` | Dashboard month totals, the monthly donor list, and month-to-date report defaults now use one server-owned date window and integer-cents arithmetic. Month trend compares the current partial month with the same point in the prior month; current-year lists exclude future-dated gifts. |
| Donor Dashboard component consolidation | Working | `app/components/dashboard/NaturalisticDonorDashboard.tsx`, `app/components/dashboard/DonorDashboardOverviewSections.tsx`, `app/components/dashboard/shared/DashboardPrimitives.tsx`, `app/features/donor-dashboard/services/dashboard-client-service.ts` | The mounted dashboard composes shared metric cards, panels, status pills, mini tiles, and live-data overview sections. Its recent gifts now open real gift details and show date-only gift dates accurately; focus cards appear only for active priorities. Remaining cleanup is tracked in [donor dashboard cleanup TODOs](../plans/donor-dashboard-cleanup-todos.md). |
| Gift detail and entry validation | Working | `app/donations/[id]/page.tsx`, `app/components/donations/DonationForm.tsx`, `server/src/routes/donations.ts`, `server/src/services/donation-tax.ts` | Gift detail deep links now open the organization-scoped saved record with attribution, payment, receipt, and follow-up details. The ledger and dashboard link to it. Add/edit gift flows reject invalid dates, choices, linked records, and fractional cents instead of silently changing entered values. |
| Calendar and fiscal report basis | Working | `app/components/donor-reports/DonorReportsSpreadsheet.tsx`, `server/src/services/donor-report-library.ts`, `server/src/routes/reports.ts` | Staff can switch report basis between calendar and fiscal years. Three-year donor analysis, lapsed-donor cohorts, dashboard summaries, giving trends, and designation summaries honor the organization fiscal-year start and label fiscal results as FY. |
| Personal user profile and usage | Working | `app/settings/profile/page.tsx`, `server/src/routes/users.ts`, `prisma/schema.prisma` | Every authenticated user can maintain their own name, preferred name, phone, title, timezone, bio, and avatar URL without changing authorization fields. The profile includes real 30-day action/sign-in/active-day metrics and recent activity derived from user-scoped audit records. |
| Donor report library | Working | `app/reports/page.tsx`, `app/components/donor-reports/DonorReportsSpreadsheet.tsx`, `server/src/services/donor-report-library.ts`, `server/src/routes/reports.ts` | Reports live inside the normal Donor CRM shell as a card library and live workbook. The library includes gifts, payment, designation, recurring-giving, and campaign-performance reports; lifetime and monthly giving; donor files, follow-up, notes, first-time/lapsed/never-given donors, top donors, batch receipt register, an unacknowledged-gifts review queue, and a printable/exportable three-year active/new/repeat donor analysis matrix. A visual chart mode, hover context, quick date ranges, and a selected-period CRM performance scorecard with transparent comparison insights are available. Donor report rows can launch a validated preselected batch in the canonical letter generation workspace. Every visible report has a live API and formula-safe CSV export; unsupported capacity values are explicitly shown only when stored as donor tags. |
| Portable full CRM backup package | Working | `app/components/watchdog/ops/WatchdogBackupsPage.tsx`, `server/src/services/crm-backup-package.ts`, `server/src/routes/watchdog-ops.ts` | Watchdog admins can download one re-importable ZIP containing the full database snapshot, SQL dump, database-stored configuration, and every local `public/uploads` asset (including media). A SHA-256 manifest validates database and asset files before a guarded restore; restore requires a typed confirmation and audit reason, creates a pre-restore database snapshot, and preserves the replaced uploads directory as a server recovery copy. Environment secrets and provider-hosted files remain intentionally excluded. |
| Personal DonorCRM appearance | Working | `app/settings/appearance/page.tsx`, `app/components/layout/AppShell.tsx`, `server/src/routes/settings.ts` | Each authenticated user can choose light-green (default), blue, violet, or slate DonorCRM command bars and comfortable/compact density. The preference is scoped to that user, persists server-side, and never changes public branding or another user’s workspace. |
| Simplified enterprise visual system | Working | `app/globals.css`, `app/components/layout/topbar/theme-tokens.ts`, `app/components/layout/TopBar.tsx` | Shared CRM surfaces use lighter neutral backgrounds, restrained borders, indigo command/focus accents, and a compact command bar with global search, quick add, notifications, Steward, help, and account tools. |
| Staff direct messages | Removed | `app/components/layout/TopBar.tsx`, `server/src/index.ts` | The internal staff-to-staff messaging panel, API, settings card, live event stream, and unread badges are retired. Historical database records are retained but unreachable from the active CRM runtime; any future replacement requires a new security, retention, and UX review. |
| Product naming and release version | Working | `package.json`, `app/layout.tsx`, `app/manifest.ts`, `server/src/lib/app-info.ts` | The CRM release is OyamaCRM v1.45b (`1.45.0-beta` package version). Compatibility identifiers, asset paths, and public embed APIs retain their existing technical names. |
| Import center and reusable mappings | Working | `app/data-tools/page.tsx`, `app/data-tools/AudienceCsvListTool.tsx`, `app/data-tools/merge/MergeWorkflow.tsx`, `app/data-tools/import/ImportWizard.tsx`, `app/components/contacts-manager/AudienceListManager.tsx`, `server/src/routes/constituents.ts`, `server/src/routes/donations.ts` | The import center exposes constituent, donation, event guest, audience-list, rollback, and duplicate-resolution paths together. Constituent imports merge repeated rows into the matched record by default and save audience membership in a verified transaction, including imported or matched CRM contacts without email when requested. Audience List Manager can also select a previous import and idempotently add its retained contacts to any saved list, which repairs an empty list without re-importing donors. Guarded rollback is available from Data Tools. Staff can match a separate CSV to existing contacts and save the unique matches as a list without creating contacts. Duplicate groups support select-all and bulk merge into each group's oldest record while preserving linked history, communication protections, groups, and saved-list membership. Donation imports retain their dry-run and exact-match safeguards. |
| Microsoft-style communication builders | Working | `app/components/email-builder/EmailBuilderApp.tsx`, `app/components/email-builder/BlockPalette.tsx`, `app/components/email-builder/EmailCanvas.tsx`, `app/components/letters/OyamaLettersWorkspace.tsx` | Email and letter authoring use a shared flat command-bar visual language with blue primary actions, neutral workspace canvases, structured ribbon/rail controls, and visible draft/review state. Existing preview, test-send, publish, and server-rendered PDF safeguards remain unchanged. |
| Trivia event-night temporary access | Working | `server/src/routes/trivia.ts`, `app/components/trivia/TemporaryEventAccessPanel.tsx`, `app/components/trivia/TriviaRemoteController.tsx`, `app/apps/trivia/remote/page.tsx` | A producer can create short-lived, revocable four-digit codes for host, scorekeeper, quick check-in, or table-and-guest-manager roles. Every new pass supplies a complete auto-connect URL, a common easy-to-type code-entry URL, and a QR code. Unauthenticated remote entry bypasses the CRM login safely; claims are rate-limited, globally resolve the event, create an event-scoped session, and keep every action role-checked and auditable. |
| Trivia public event registration and invitations | Working | `server/src/routes/trivia.ts`, `server/src/routes/events.ts`, `app/components/trivia/TriviaPublicRegistrationPage.tsx`, Event Studio public registration | Trivia creation publishes a linked Event Studio page, which is the sole write path for public team/table registration. Legacy Trivia URLs redirect to that page; an unlinked legacy event fails closed until migrated. The relational serializable transaction owns capacity, orders, tables, seats, guests, check-in codes, confirmation email, and verified Stripe/offline payment state. The reviewed invitation sender remains preference-aware and auditable. |
| Trivia canonical table and member operations | Working | `app/components/trivia/ops/TriviaCheckInWorkspace.tsx`, `app/apps/trivia/hooks/useTriviaModuleState.ts`, `app/apps/trivia/lib/trivia-store.ts`, `server/src/routes/trivia.ts` | Every team receives one canonical numeric table number from 1–9999. Public signup, walk-ins, local administration, whole-state sync, and the table-manager remote normalize the identifier and prevent duplicates. Run the Night → Check-In displays every named table member and provides one reviewed editor for team, table host, contacts, payer, payment state, amount, notes, and member roster; duplicate or invalid table numbers are rejected without overwriting the existing assignment. |
| Trivia projector and host connection safety | Working | `app/apps/trivia/events/[eventId]/host/page.tsx`, `app/apps/trivia/display/[eventId]/page.tsx` | The host panel makes server sync a visible event-night prerequisite and reports connection state. Projector pages request server sync on load so they follow the shared event state instead of silently relying on a stale local browser copy. |
| Trivia visual builder and question media | Working | `app/components/trivia/TriviaGameMap.tsx`, `app/apps/trivia/events/[eventId]/builder/page.tsx`, `app/apps/trivia/hooks/useTriviaModuleState.ts`, `app/components/trivia/ProjectorDisplayView.tsx`, `server/src/routes/trivia.ts` | The builder is a dedicated visual game-map workspace with a component rail, connected round lanes, drag-to-reorder questions and rounds, and a contextual inspector. Producers can edit Welcome-screen copy and visibility, edit or delete rounds safely, and edit, duplicate, or delete questions; changes persist to the same local/server state used by host and projector views. Image, audio, and video questions accept direct links or authenticated uploads. The audience display uses one animated navy/cyan show package across all public stages, with reduced-motion and high-contrast support. Temporary host, check-in, and scorekeeper roles are event-scoped and revocable. |
| Portable Trivia game import | Working | `app/events/trivia-import/page.tsx`, `app/apps/trivia/lib/trivia-game-import.ts`, `public/trivia-games/pcc-through-the-decades.json` | Events and the trivia builder expose a prominent Import trivia game action. Producers upload a versioned JSON game file, review all rounds and answers, and append the validated content to a draft event in one state update. The PCC Through the Decades package contains four decade rounds and a ten-question tie breaker. |
| EventSTUDIO table organizer integrity | Working | `app/events/tables/page.tsx`, `server/src/services/event-table-service.ts`, `server/src/services/event-seat-service.ts`, `server/src/routes/events.ts`, `prisma/migrations/20260729021500_event_table_number_uniqueness_and_types/migration.sql`, `tests/smoke/events-crud.test.ts` | Tables have unique event-scoped numbers, bounded capacities, synchronized seat records, and searchable table/host/guest operations. Seat assignments are transactional: full tables and occupied seats return actionable conflicts, moving a guest releases the old seat, unassigning clears stale seat fields, and tables cannot be reduced below their assigned roster. Table deletion uses an in-app review step and preserves guests as unassigned event registrants. |
| EventSTUDIO public RSVP and table purchase | Working | `app/components/events/public/PublicEventRegistrationForm.tsx`, `server/src/routes/events.ts`, `tests/smoke/events-crud.test.ts` | Published event pages support individual or fixed-seat table registration. Public submissions are rate-limited, retry-idempotent, status/deadline gated, and recheck event capacity, ticket capacity, and ticket availability inside a serializable transaction. Identical retries return the original order without duplicate guests, seats, inventory decrement, or email; changed retries fail safely with a support reference. Table purchases create a durable numbered table, seats, host, order, constituent, guests, and one check-in code per attendee. |
| EventSTUDIO event-first workspace | Working | `app/components/events/EventsStudioShell.tsx`, `app/events/[eventId]/overview/page.tsx`, `app/events/setup/page.tsx` | The Events shell uses one compact event-scoped navigation model across overview, RSVPs, tables, check-in, public page, fundraising, outreach, reports, and settings. The overview reads the real public-page publish state, table numbering/capacity conflicts, check-in codes, host coverage, payments, and seating readiness. The retired mock setup route now redirects to the live event selector instead of presenting placeholder readiness. |
...
| Relationship-focused donor profile | Working | `app/constituents/[id]/page.tsx`, `app/components/ui/crm/ribbon/config.ts` | Profiles foreground giving, distinct open work, risk, opportunity, relationship context, and explicit outreach safeguards. Email, phone, mail, ribbon, and contextual AI actions fail closed when contact data or preferences block that channel; duplicate task records are grouped while retaining the source count. |
...
| Area | Status | Evidence | Notes |
|---|---|---|---|
| Image uploads inside structured columns | Working | `app/components/oyama-email/OyamaEmailBuilderWorkspace.tsx`, `app/components/email-builder/BlockEditor.tsx`, `server/src/routes/email-campaigns.ts`, `server/src/routes/letters.ts`, `server/src/routes/settings.ts`, `tests/smoke/oyama-email-workspace-source.test.ts` | Image and video-thumbnail controls at every nested column depth use the same validated campaign-media uploader as top-level blocks. The single right inspector exposes each selected column image with a direct upload action. Nested updates traverse the saved block tree instead of stopping at the top-level canvas. Email images, letter-editor images, handwritten signatures, and shared branding logos validate both their declared type and their real PNG/JPEG/WEBP/GIF bytes before storage. |
...
| Organized email builder | Working | `app/components/oyama-email/OyamaEmailBuilderWorkspace.tsx`, `tests/smoke/oyama-email-workspace-source.test.ts` | The compact builder bar keeps workflow, readiness, zoom, and managed email-chrome guidance available without consuming canvas height. A `+ Add block` modal groups Content, Layout, Engagement, and ready-made sections without reserving a permanent left rail. Canvas insertion points place blocks precisely, while the right inspector is the single block-editing source. |
| Letter block library, compact tool drawers, and data drawers | Working | `app/components/letters/OyamaLettersWorkspace.tsx`, `tests/smoke/letter-builder-ui-source.test.ts` | The letter canvas retains its production-PDF document editor while replacing long sidebar lists with Writing, Layout, and Giving & sign-off block drawers. File, Insert, Format, Layout, Review, and View controls now open a compact on-demand drawer rather than a persistent ribbon; readiness is summarized in the command bar. Steward AI stays closed until its violet chat control is selected. The responsive builder uses compact indigo/blue surfaces and tighter sidebars. |
| Reorganized email and letter template libraries | Working | `app/components/oyama-email/OyamaEmailWorkspace.tsx`, `app/components/letters/OyamaLettersWorkspace.tsx`, focused workspace source tests | Both template libraries now use a concise action header, dedicated browse/filter rail, focused search and sort controls, live category counts, and a clear results summary. Email makes the distinction between reusable content editing and campaign use explicit; letters distinguish editing a draft from continuing to recipient-specific generation. |
| Global communication branding defaults | Working | `app/settings/branding/page.tsx`, `app/components/letters/LetterBrandingManager.tsx`, `app/components/letters/OyamaLettersWorkspace.tsx`, `server/src/routes/letters.ts` | Settings → Branding → Communication Header + Footer is the source of truth for managed email chrome and printed-letter header/footer defaults. The selected global header now takes precedence over legacy template preset links in canvas, sample preview, generated exports, and batch PDFs. Recipient mode puts the donor name/address in the upper right and removes the duplicate body address. Branding Settings explicitly identifies the active global header and offers a one-click “Use as global header” action. |
...
| Area | Status | Evidence | Notes |
|---|---|---|---|
| Final letter hierarchy and configurable header | Working | `app/components/letters/LetterPage.tsx`, `app/components/letters/LetterBrandingManager.tsx`, `server/src/routes/letters.ts`, `prisma/migrations/20260722153000_letter_header_right_column/migration.sql` | The uploaded logo is the upper-left wordmark without a duplicate organization name. Header presets choose organization details, recipient details, or custom merge-field text for the upper right. The server uses intentionally separated address/contact lines, right alignment, and the same compact header geometry as the canvas. |
| Saved page-layout source of truth | Working | `app/components/letters/OyamaLettersWorkspace.tsx`, `server/src/routes/letters.ts`, `server/src/services/letters-execution.ts` | The canvas page size and margins persist under `letterPdfLayout`; server PDF preview uses unsaved settings, and generated letters snapshot that layout for later download and batch print. New documents use 0.25-inch margins. |
| PDF table fidelity | Working | `server/src/routes/letters.ts`, `tests/unit/letters-pdf-layout.test.ts` | Server PDFs preserve table borders, header fill, padding, bold headers, and per-cell alignment for donation-summary and editor-wrapped tables; wrapped tables are extracted before generic HTML parsing so they cannot collapse into body text. |
| Unified production preview and print | Working | `app/components/letters/LetterPrintRoute.tsx`, `app/components/letters/OyamaLettersWorkspace.tsx`, `server/src/routes/letters.ts` | The print route, template/publish preview, generation preview, exports, and queue batch all use the server-rendered production PDF; footer organization text is de-duplicated when the legal line already contains it. |
| Automatic pages and deliberate breaks | Working | `server/src/routes/letters.ts`, `app/components/letters/OyamaLettersWorkspace.tsx`, `tests/unit/letters-pdf-layout.test.ts` | Long content automatically continues on the next page. One-page letters show both header and footer; multi-page letters show the header only on page 1 and the footer only on the final page. The editor exposes an Add Page action when staff want to choose the break point. |
...
Validation: focused Letter PDF suite passed 26/26, including saved 0.25-inch page layout and editor-wrapped table regressions; web/server typechecks passed. A representative full donor letter PDF was rendered and visually inspected against the canvas contract.
...
| Area | Status | Evidence | Notes |
|---|---|---|---|
| Letter generation workflow | Working | `app/components/letters/OyamaLettersWorkspace.tsx`, `tests/smoke/letter-builder-ui-source.test.ts` | The normal path is four visible stages: Setup, Recipients, Preview, Generate. Gift context remains optional, recipient selection proceeds directly to a server-rendered preview, and project settings can be saved on the selected template for later reuse. |
| Letter preview and output controls | Working | `app/components/letters/OyamaLettersWorkspace.tsx` | Preview exposes the server-rendered production PDF plus individual, ZIP, and batch downloads. Generate is PDF-only; validation, reopen, refresh, and a new-run action remain under Advanced Generation Tools. |
...
| OyamaEmail template studio | Working | `app/components/oyama-email/OyamaEmailBuilderWorkspace.tsx`, `tests/smoke/oyama-email-workspace-source.test.ts` | Modal block insertion, ordering, recipient preview, plain text, autosave, test send, one right-side block inspector, compliance settings, and publish routing are retained. A live readiness score and warning summary make incomplete requirements visible without hiding the canvas. |
| Browser-backed live workflow audit | Working for donor profile | Logged-in local browser audit on 2026-08-30 | Donor profile controls, safeguard status, duplicate-task grouping, and layout were inspected with live data at 1440×900 and 390×844. No document-level horizontal overflow or browser console errors were found. Other release workflows retain their own validation status. |
...
| Area | Status | Evidence | Notes |
|---|---|---|---|
| Top bar Steward entry point | Working | `app/components/layout/TopBar.tsx`, `app/components/ai/StewardDockPanel.tsx` | Removed the floating Steward chat-head launcher from the CRM shell and retained a compact Steward workspace entry point. |
...
| Area | Status | Evidence | Notes |
|---|---|---|---|
| Donor report library | Working | `app/components/donor-reports/DonorReportsSpreadsheet.tsx`, `server/src/services/donor-report-library.ts` | The prior standalone report builder and chart viewer were retired. Donor CRM now exposes a live report library with date/year filters, safe export, focused printing, and a spreadsheet-style three-year donor analysis matrix. |
...
| Runtime system diagnostics | Working | `app/settings/system/page.tsx`, `app/settings/system-status/page.tsx`, `app/settings/project-status/page.tsx` | The internal readiness and project-audit dashboard was removed from the product UI. System settings retains version and live service diagnostics, while both retired URLs redirect there for bookmark compatibility. |
...
| Guided HubSpot audience import | Working | `app/components/data-tools/GuidedImportWizard.tsx`, `app/data-tools/import/fieldMap.ts`, `app/data-tools/import/ImportWizard.tsx`, `server/src/routes/constituents.ts` | Data Tools now asks donor/outreach vs Compassion client before import, supports a HubSpot contacts preset, maps HubSpot Email Lists into tags, and creates Contacts Manager audiences from all imported or matched contacts or only email-ready contacts. Newsletter/Churches/Businesses/Organizations segments are created from email-ready rows. |
...
| Donor CRM | Donations CRUD + donation import | Working | Real API Data | `app/donations/page.tsx`, `server/src/routes/donations.ts`, and `/api/donations/import` are wired. Gift entry and edit capture a partial tax-deductible amount, donor receipt request, and tax notes; receipt merge fields use the stored amount. The new `year.taxDeductibleTotal` merge field totals eligible portions while `year.totalGiving` remains gross. Existing gifts retain the full-or-zero interpretation after migration. Receipt preparation opens OyamaLetters with the selected gift and never sends from the gift form. Imports verify receipt and transaction IDs against existing records, surface—not silently merge—same-donor/date/amount candidates, and use audited file fingerprints to require explicit confirmation before repeating a committed source file. | Add rollback support and receipt automation. |
...
| Donor CRM | Letters & Printables workspace | Partially Working | Real API Data | `/api/letters` plus `/letters-printables` now support template CRUD, rich letter authoring, merge preview, single-letter generation, server-side PDF export (single and batch), unified queue workspace routing at `/letters-printables/queues`, print queue actions, mail queue actions, timeline logging, email draft creation, and enforced workflow policy settings via `/api/letters/workflow-settings` in execution lanes. Production PDFs keep the canvas's Arial/Helvetica base family, retain merged salutations, keep automatic signatures in normal flow, place the generated/current letter date in the top-right body position, consume a leading gift-date line to prevent duplicate left-side dates, and retain intentional editor spacing. Builder and Publish Review previews are single-flight, time-bounded, request-correlated, and visibly recoverable. | Improve mixed inline PDF fidelity/storage delivery metadata and expand print-vendor handoff automation. |
...
| Donor CRM | Reports | Working | Real API Data | `app/reports/page.tsx` renders a live Donor CRM report library backed by `GET /api/reports/library/:reportKey`, including gift, donor, and three-year comparison reports with authorized CSV export and focused printing. | Add report authoring/saved custom layouts only when their live data contracts and export paths are ready. |
...
| Donor CRM | Stripe Giving integration and payment ledger | Working | Real API Data | `/integrations/stripe`, `/give/[token]`, `HostedGivingPage.tsx`, `StripeIntegrationWorkspace.tsx`, `payment-gateway-settings.ts`, `server/src/routes/payments.ts`, `server/src/routes/site-embeds.ts`, `PaymentWebhookEvent`, and `/payments`; admins securely save independent encrypted Test and Live Stripe credential sets, test either account without switching public traffic, open an isolated Embedded Checkout sandbox form, publish a direct OyamaCRM-hosted giving link, build/install a fail-closed domain-restricted external form, verify its loader on the exact public hostname, select standard/compact/warm/campaign embed variants, configure preset plus “Other” amounts, min/max limits, cadence, donor fields, CTA copy, and visual treatment, inspect verified webhook events, and review the resulting live donation ledger. Checkout objects carry durable organization routing metadata alongside the site token, so a signed delivery can still reach the correct ledger after a token change; the workspace distinguishes saved credentials from an actually received verified delivery. Hosted and embedded semantic HTML forms share the same server validation and signed one-time, delayed-success, and recurring webhook pipeline; verified events idempotently create real Donation, donor activity, audit, and giving-summary records while failures remain visible without creating false gifts. | Apply the payment-event migration, register the five displayed webhook events in both Stripe environments, complete one-time and monthly sandbox proofs through both public delivery modes, verify every external hostname, and perform one low-dollar live proof gift with the production Stripe account before public launch. |
| Platform | Version/build/status visibility | Working | Real API Data | `/api/health` and the settings System page show version/build metadata and live service diagnostics. | Add release notes/changelog UI and deployment history. |
...
| donor.reports | Working | `app/reports/page.tsx` + `GET /api/reports/library/:reportKey` | A live Donor CRM report library is available with gift and donor report cards, server-authorized CSV export, focused printing, and a spreadsheet-style three-year donor analysis worksheet. |
...
- The Trivia tab is now the night-of command center: it reports preflight readiness and shared-sync safety, shows the current live stage, and launches role-specific check-in, host, scorekeeper, judge, projector, scoreboard, print, and recovery workspaces without leaving the Event route family.
- Unified Trivia routes resolve the EventSTUDIO id, canonical Trivia id, or retained legacy Trivia id so linked records cannot strand staff on an “Event not found” screen during setup or show time.
...
- Theme-coherence follow-up: every unified Trivia administration route now receives the Event Studio light/indigo theme boundary. Legacy navy panels, near-black inputs, neon fuchsia actions, low-contrast pastel labels, and the dark visual builder frame are translated to accessible light surfaces and one indigo action language. Projector and temporary phone-remote surfaces remain intentionally dark for distance and event-floor use.
