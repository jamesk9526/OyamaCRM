...
| Import center and reusable mappings | Working | `app/data-tools/import/page.tsx`, `app/components/data-tools/GuidedImportWizard.tsx`, `app/data-tools/import/ImportWizard.tsx`, `app/data-tools/import/DonationImportWizard.tsx`, `server/src/routes/donations.ts` | The import center exposes constituent, donation, event guest, and duplicate-resolution paths together. Donation imports support dry runs, source-file fingerprints, receipt/transaction-ID matching, reviewable same-donor/date/amount candidates, and audit history. Existing donations are preserved unless an authorized staff member explicitly elects to update an exact match. |
...
| Trivia public event registration, invitations, and payment handoff | Working | `server/src/routes/trivia.ts`, `app/components/trivia/TriviaRegistrationSettingsPanel.tsx`, `app/components/trivia/TriviaPublicRegistrationPage.tsx`, `app/apps/trivia/events/[eventId]/registration/page.tsx`, `app/trivia/[slug]/page.tsx` | Producers can customize and publish a public Trivia event page, open or close signup, enforce table and seat capacity, collect team names, table hosts, contact details, named members, payer details, and accessibility notes, and choose free, per-seat, per-table, or mixed pricing. A reviewed invitation sender accepts one address or pasted lists, requires permission confirmation, enforces constituent preferences/suppressions, and reports sent/skipped/failed recipients. Successful RSVPs receive a transactional confirmation containing the canonical table number, member roster, four-digit check-in code, amount due, and payment link/instructions. Public submissions are server-validated, rate-limited, added to the shared event roster, and protected from stale browser-state overwrites. Paid registrations can hand off to HTTPS Stripe or PayPal hosted links or retain offline-payment instructions; status remains explicitly pending until verified by staff. |
...
| EventSTUDIO public RSVP and table purchase | Working | `app/components/events/public/PublicEventRegistrationForm.tsx`, `server/src/routes/events.ts`, `tests/smoke/events-crud.test.ts` | Published event pages support individual or fixed-seat table registration. Public submissions are rate-limited and recheck event capacity, ticket capacity, and ticket availability inside a serializable transaction. Table purchases create a durable numbered table, seats, host, order, constituent, guests, and one check-in code per attendee. After persistence, the organization email provider sends a compliance-checked transactional confirmation; the on-screen receipt remains authoritative and clearly preserves order/table/check-in details when email is unavailable. |
