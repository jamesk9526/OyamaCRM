...
| Saved base lists can be edited directly | Working | `app/components/contacts-manager/AudienceListManager.tsx`, `ContactsManagerPage.tsx`, `DELETE /api/email-campaigns/lists/:listId/recipients/:memberId` | List Manager searches the complete saved membership and removes one person atomically from the shared base list with audit logging. Segment Builder clearly identifies base-list edit mode and the save action that commits bulk additions/removals. |
...
| Segment Builder direct list removal | Working | `app/components/contacts-manager/ContactsManagerPage.tsx`, `DELETE /api/email-campaigns/lists/:listId/recipients/:memberId` | Selected constituent rows in both the Segment Builder table and Selected People panel expose a labeled Remove from list action. When editing a saved base list, removal is committed immediately and the shared membership/count is updated in place; unsaved new-segment changes remain local until save. |
