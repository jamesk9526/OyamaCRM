...
| Donor CRM | Stripe Giving integration and payment ledger | Working | Real API Data | `/integrations/stripe`, `StripeIntegrationWorkspace.tsx`, `server/src/routes/payments.ts`, `server/src/routes/site-embeds.ts`, `PaymentWebhookEvent`, and `/payments`; admins securely configure and test Stripe, build/install a domain-restricted Embedded Checkout form, inspect verified webhook events, and review the resulting live donation ledger. Signed one-time and recurring Stripe payment events idempotently create real Donation, donor activity, audit, and giving-summary records. | Apply the new migration, register the displayed webhook URL, and perform one low-dollar live proof gift with the production Stripe account before public launch. |
