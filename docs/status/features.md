...
| Donor CRM | Donations CRUD + donation import | Working | Real API Data | `app/donations/page.tsx`, `server/src/routes/donations.ts`, and `/api/donations/import` are wired. Imports verify receipt and transaction IDs against existing records, surface—not silently merge—same-donor/date/amount candidates, and use audited file fingerprints to require explicit confirmation before repeating a committed source file. | Add rollback support and receipt automation. |
