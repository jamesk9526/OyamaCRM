...
| Trivia visual builder and question media | Working | `app/components/trivia/TriviaGameMap.tsx`, `app/apps/trivia/events/[eventId]/builder/page.tsx`, `app/apps/trivia/hooks/useTriviaModuleState.ts`, `app/components/trivia/ProjectorDisplayView.tsx`, `server/src/routes/trivia.ts` | The builder is a dedicated visual game-map workspace with a component rail, connected round lanes, drag-to-reorder questions and rounds, a contextual question inspector, focused creation dialogs, and automatic persistence to the same local/server-synced event state used by host and projector views. Image, audio, and video questions accept direct links or authenticated uploads. The audience display uses one animated navy/cyan show package across welcome, check-in, rounds, questions, media, timer, answers, leaderboard, breaks, and winner screens, with reduced-motion and high-contrast support. Temporary host, check-in, and scorekeeper roles are event-scoped and revocable. |
