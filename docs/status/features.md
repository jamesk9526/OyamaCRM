...
_Last focused audit: 2026-07-29 (Donor CRM month-to-date reporting workbook)_
...
| Donor reporting workbook | Working | `app/reports/page.tsx`, `app/components/donor-reports/DonorReportsSpreadsheet.tsx`, `server/src/routes/reports.ts` | Reports now live inside the normal Donor CRM shell rather than a separate Reports app. The first live workbook sheets are Month-to-date giving and Donors by designation; both use completed gifts from the first day of the current month through now. The donor/designation sheet sorts in place, opens live donor records, prints a focused worksheet, and exports a server-authorized, formula-safe CSV. Legacy `/reports/builder`, `/reports/manager`, and `/reports/donor-crm` links redirect to the canonical workbook. |
...
| Area | Status | Evidence | Notes |
|---|---|---|---|
| Donor reporting workbook | Working | `app/components/donor-reports/DonorReportsSpreadsheet.tsx` | The prior standalone report builder and chart viewer were retired. Donor CRM now exposes a focused workbook with real month-to-date giving and donor-by-designation sheets, in-place sorting, safe export, and focused printing. |
...
| Donor CRM | Reports | Working | Real API Data | `app/reports/page.tsx` renders the Donor CRM reporting workbook backed by `GET /api/reports/donors-by-designation`. Its first two sheets show MTD giving totals and donor-by-designation rows, with a server-authorized CSV export and a focused printable sheet. | Add new workbook sheets (campaign, retention, and custom date ranges) only when their live data contracts and export paths are ready. |
...
| donor.reports | Working | `app/reports/page.tsx` + `GET /api/reports/donors-by-designation` | A focused Donor CRM reporting workbook is live with month-to-date giving and donor-by-designation sheets, server-authorized CSV export, and worksheet printing. |
