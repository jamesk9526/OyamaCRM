...
| Import center and reusable mappings | Working | `app/data-tools/page.tsx`, `app/data-tools/AudienceCsvListTool.tsx`, `app/data-tools/merge/MergeWorkflow.tsx`, `app/data-tools/import/ImportWizard.tsx`, `server/src/routes/constituents.ts`, `server/src/routes/donations.ts` | The import center exposes constituent, donation, event guest, audience-list, rollback, and duplicate-resolution paths together. Constituent imports merge repeated rows into the matched record by default and can save either every imported or matched CRM contact—including those without email—or only email-ready contacts to a requested audience list with the import run. Guarded rollback is available from Data Tools. Staff can match a separate CSV to existing contacts and save the unique matches as a list without creating contacts. Duplicate groups support select-all and bulk merge into each group's oldest record while preserving linked history, communication protections, groups, and saved-list membership. Donation imports retain their dry-run and exact-match safeguards. |
