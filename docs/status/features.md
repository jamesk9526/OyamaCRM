...
| Trivia visual builder and question media | Working | `app/components/trivia/TriviaGameMap.tsx`, `app/apps/trivia/events/[eventId]/builder/page.tsx`, `app/apps/trivia/hooks/useTriviaModuleState.ts`, `app/components/trivia/ProjectorDisplayView.tsx`, `server/src/routes/trivia.ts` | The builder is a dedicated visual game-map workspace with a component rail, connected round lanes, drag-to-reorder questions and rounds, a contextual question inspector, focused creation dialogs, and automatic persistence to the same local/server-synced event state used by host and projector views. Image, audio, and video questions accept direct links or authenticated uploads (images up to 5MB; supported audio/video up to 12MB), with projector-safe rendering. Event templates, roster tools, and JSON backups are available in its secondary setup panel. |
