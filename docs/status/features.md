...
| Trivia visual builder and question media | Working | `app/components/trivia/RoundQuestionBuilderPanel.tsx`, `app/components/trivia/TriviaGameMap.tsx`, `app/components/trivia/ProjectorDisplayView.tsx`, `server/src/routes/trivia.ts` | Image, audio, and video questions can use a direct media link or an authenticated upload (images up to 5MB; supported audio/video up to 12MB). The builder shows file previews and a visual round/question game map; projector questions render the selected media. Trivia setup uses the purple-and-white administrative visual treatment. |
