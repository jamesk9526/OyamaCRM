...
| Global communication branding defaults | Working | `app/settings/branding/page.tsx`, `app/components/letters/LetterBrandingManager.tsx`, `app/components/oyama-email/OyamaEmailBuilderWorkspace.tsx`, `app/components/letters/OyamaLettersWorkspace.tsx` | Settings → Branding → Communication Header + Footer is the source of truth for managed email chrome and printed-letter header/footer defaults. Letter headers can globally use organization contact details, donor/recipient name and address, or custom merge-field content in the upper right; recipient mode removes the duplicate body address and makes more room for the letter body. Template editors link to these inherited defaults instead of offering conflicting header/footer controls. |
