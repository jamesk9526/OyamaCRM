...
| Letter block library and data drawers | Working | `app/components/letters/OyamaLettersWorkspace.tsx`, `tests/smoke/letter-builder-ui-source.test.ts` | The letter canvas retains its production-PDF document editor while replacing the long sidebar lists with Writing, Layout, and Giving & sign-off block drawers. Content, tables, gift summaries, signatures, images, spacers, and page breaks can be clicked or dragged into the letter; the initially closed Data & Merge Fields drawer groups live CRM fields by category. |
