...
| Direct public giving works without embed installation | Working | `/give/[token]`, `/api/giving/[token]`, `HostedGivingPage.tsx`, and the public donation-page API publish an optional shell-free OyamaCRM giving URL. The semantic responsive HTML form supports one-time/monthly gifts, presets and custom amounts, designations, donor identity, Test-mode disclosure, and Stripe Embedded Checkout while reusing the verified webhook ledger path. Hosted-only publication does not weaken external embed domain enforcement. |
