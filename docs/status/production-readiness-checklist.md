...
| Large pre-generation PDF downloads complete within gateway request limits | Working | `app/components/letters/OyamaLettersWorkspace.tsx`, `server/src/routes/letters.ts`; large recipient sets are prepared sequentially in 40-recipient parts, expose a native save link for every completed part, and use bounded server merge/render concurrency. |
