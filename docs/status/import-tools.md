...
|---|---|---|---|---|---|
| Data Tools | Constituent import wizard | Working | Real API Data | `app/data-tools/import/ImportWizard.tsx` posts to `/api/constituents/import` with dry-run support. Live imports now write run metadata, expose `/api/constituents/import/history`, and support guarded rollback via `/api/constituents/import/:runId/rollback` after preview checks and explicit typed confirmation. Auto-map now uses normalized header matching (punctuation/spacing variants), and import mapping now supports direct preference flags (`doNotEmail`, `doNotCall`, `doNotMail`, `doNotContact`, `emailOptOut`) plus `country`. | Expand rollback to donation and Compassion import flows with matching safety guards. |
...
| Data Tools | Donation import wizard | Working | Real API Data | `DonationImportWizard.tsx` posts to `/api/donations/import`; the endpoint links constituents/campaigns/designations and recognizes in-file duplicates, exact receipt/transaction-ID matches already in Oyama, ambiguous exact-ID collisions, and reviewable same-donor/date/amount candidates. Potential matches are never asserted to be duplicates and are paused by default. Audited SHA-256 source fingerprints require an explicit acknowledgement before a committed file can be imported again. Auto-map aliases include eKYROS `Donations_List` headers (e.g. `FormDate1`, `AmountReceived`, `PaymentTypeDesc`, `EventName`). | Add rollback tooling for donation imports. |
| Data Tools | Field mapping engine | Working | Real API Data | `fieldMap.ts` + `donationFieldMap.ts` + `compassionFieldMap.ts` provide mapping aliases and required-field behavior, including real export headers from office datasets. Constituent and donation mappers now also check normalized header candidates (raw, normalized, compact, underscore/dash variants) before falling back to skip. | Keep maps synchronized whenever schema fields are added. |
...
- **Missing:** rollback/undo parity is still missing for donation and Compassion import flows.
